...
export default function CallLogReport({ logs, callerId }: { logs: CallLog[]; callerId: string }) {
  ...
  return <>
    ...
    <section className="call-report">
      ...
      <div className="call-log-body">{filtered.map(log => <div className="call-log-row" key={log.id}><span><b>Alex Carranza</b><small>{log.name}</small></span><span>{new Date(log.startedAt).toLocaleString([], { month:"short", day:"numeric", hour:"numeric", minute:"2-digit" })}</span><span><em className={`result ${log.outcome.toLowerCase().replaceAll(" ","-")}`}>{log.outcome}</em><small>{log.errorCode || log.status}</small></span><span className="recording-cell">{log.recordingUrl ? <audio controls preload="none" src={log.recordingUrl}/> : <button disabled title="Enable consent-based Twilio recording first">▶</button>}{log.aiSummary&&<details><summary>Pacifica AI notes</summary><pre>{log.aiSummary}</pre>{log.transcript&&<p>{log.transcript}</p>}</details>}</span><span>{duration(log.duration)}</span><span>{log.phone}</span></div>)}{!filtered.length && <div className="empty-call-log"><b>No matching calls yet</b><span>Place a call and its Twilio status will appear here.</span></div>}</div>
    </section>
