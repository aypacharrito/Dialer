...
export default function CallLogReport({ logs, callerId, agentName, recordingEnabled, onOpenRecordingSettings }: { logs: CallLog[]; callerId: string; agentName:string; recordingEnabled:boolean; onOpenRecordingSettings:()=>void }) {
  ...
  return <>
    ...
    <section className="recording-readiness ready"><div><span>PER-CALL RECORDING READY</span><b>Give the required disclosure, then press Record on the live call.</b><small>Pacifica never records silently. New recordings appear here after Twilio finishes processing them; calls made before recording was started cannot be recovered.{recordingEnabled?" Recording reminders are enabled for this workspace.":" The owner can enable recording reminders and optional AI summaries in Settings."}</small></div><button type="button" onClick={onOpenRecordingSettings}>Recording preferences</button></section>
