...
export default function AutomationStudio({profile,onChange}:{profile:WorkspaceProfile;onChange:(profile:WorkspaceProfile)=>void}){
  ...
  useEffect(()=>{let active=true;void fetch("/api/automation/run",{cache:"no-store"}).then(async response=>{if(!response.ok)throw new Error("Schedule unavailable");return response.json()}).then(data=>{if(!active)return;if(typeof data.browserSchedule!=="string")throw new Error("Schedule unavailable");setMessage(data.configured&&typeof data.serverSchedule==="string"?`${data.browserSchedule}. Server backup: ${data.serverSchedule}.`:data.browserSchedule)}).catch(()=>{if(active)setMessage("Runs every five minutes while Pacifica is open. Server schedule unavailable.")});return()=>{active=false}},[]);
  ...
  async function runNow(){setRunning(true);setRunMessage("Checking due follow-ups…");try{const response=await fetch("/api/automation/run",{method:"POST"});const data=await response.json();if(!response.ok)throw new Error(data.error||"Automation run failed");setRunMessage(automationRunFeedback(data))}catch(error){setRunMessage(error instanceof Error?error.message:"Automation run failed")}finally{setRunning(false)}}
  return <section className="automation-studio">
    ...
    <div className="automation-safety-row"><label><input type="checkbox" checked={profile.providerFallbackEnabled} onChange={event=>onChange({...profile,providerFallbackEnabled:event.target.checked})}/><span><b>Channel fallback</b></span></label><p role="status" aria-live="polite">{runMessage||message}</p></div>
    <details className="automation-sequence-editor"><summary>Edit sequences <span>{profile.automationSequences.length}</span></summary>{!profile.automationSequences.length&&<p className="automation-empty">No sequences configured. <button type="button" onClick={()=>onChange({...profile,automationSequences:defaultAutomationSequences.map(sequence=>({...sequence,active:false}))})}>Add starter sequences</button></p>}<div className="automation-sequences">{profile.automationSequences.map(sequence=><article key={sequence.id} className={sequence.active?"active":""}>
