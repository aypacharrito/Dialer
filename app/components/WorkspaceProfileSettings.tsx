...
export default function WorkspaceProfileSettings({profile,onChange}:{profile:WorkspaceProfile;onChange:(profile:WorkspaceProfile)=>void}){
  ...
  return <section className="workspace-profile-settings">
    ...
    <div className="workspace-profile-fields">
      ...
      <label className="wide-field">What does this business do?<textarea value={profile.businessDescription} onChange={event=>update({businessDescription:event.target.value})} placeholder="A short description of your business"/></label>
      <label className="wide-field">Products / services<ListInput values={profile.productsServices||[]} onChange={values=>update({productsServices:values})} placeholder={example.products}/></label>
      <label>Ideal customer<input value={profile.idealCustomer} onChange={event=>update({idealCustomer:event.target.value})} placeholder="Who do you help?"/></label>
      <label>Value proposition<input value={profile.valueProposition} onChange={event=>update({valueProposition:event.target.value})} placeholder="What makes your service useful?"/></label>
      <label className="wide-field">Primary sales objective<textarea value={profile.salesObjective} onChange={event=>update({salesObjective:event.target.value})} placeholder="What should the next conversation achieve?"/></label>
      ...
      <label className="wide-field">Custom AI instructions<textarea value={profile.customAiInstructions} onChange={event=>update({customAiInstructions:event.target.value})} placeholder="Tone, preferences, and details Pacifica should remember"/></label>
    ...
    <details className="profile-examples"><summary>View writing examples</summary><p><b>Business:</b> {example.description}</p><p><b>Ideal customer:</b> {example.ideal}</p><p><b>AI instructions:</b> {example.instructions}</p></details>
    <section className="lead-form-permissions" aria-label="Lead-form permissions"><h3>Lead-form permissions</h3><p>Reuse permission already collected on your lead forms. Enter the exact source names for each channel. STOP, unsubscribe, and Do Not Call still block outreach.</p><div className="workspace-profile-fields"><label>Sources with SMS consent<ListInput values={profile.smsConsentSources||[]} onChange={values=>update({smsConsentSources:values})} placeholder="SmartFinancial, Website"/></label><label>Sources with email permission<ListInput values={profile.emailConsentSources||[]} onChange={values=>update({emailConsentSources:values})} placeholder="SmartFinancial, Website"/></label></div></section>
