...
export type MessageLead={id:number;name:string;phone:string;email:string;product:string;city:string;line:"life"|"home-auto";notes:string;outcome:string;doNotCall:boolean;smsConsent?:boolean;smsOptOut?:boolean;lastSmsAt?:string;emailConsent?:boolean;emailOptOut?:boolean;lastEmailAt?:string;communications?:StoredCommunication[]};
...
function browserDraft(lead:MessageLead,profile:WorkspaceProfile,channel:Channel){
  ...
  if(channel==="email")return {subject:`Following up about your ${lead.product||"request"}`,body:`Hi ${firstName(lead.name)},\n\nThis is ${sender}. I’m following up about your ${lead.product||"service"} request${place}. If you still want help, reply to this email${callback}.\n\nBest,\n${profile.emailSignature||profile.agentName||profile.businessName||"The team"}`};
  return {subject:"",body:`Hey ${firstName(lead.name)}, it’s ${sender}. I’m following up about your ${lead.product||"service"} request${place}. If you still want help, text me here${callback}. Reply STOP to opt out.`};
}
...
export default function MessagesCenter({workspaceId,profile,leads,onPatch,onProfileChange}:{workspaceId:string;profile:WorkspaceProfile;leads:MessageLead[];onPatch:(id:number,patch:Partial<MessageLead>)=>void;onProfileChange:(profile:WorkspaceProfile)=>void}){
  ...
  return <div className="messages-center">
    <header className="messages-title"><div><span>UNIFIED COMMUNICATIONS</span><h1>Text and email from one lead timeline.</h1><p>Pacifica owns the workflow; Twilio, Resend, Telnyx, or a custom adapter can provide delivery.</p><div className={`message-connection ${connectionClass}`}><i/>{status}<button type="button" onClick={()=>void load()}>Check channels</button></div></div><label className="daily-sms-toggle"><input type="checkbox" checked={profile.serverAutomationEnabled} disabled/><span><b>Multi-channel automation {profile.serverAutomationEnabled?"enabled":"off"}</b><small>Channel consent, DNC, provider, and suppression gates enforced</small></span></label></header>
    ...
    <div className="messages-layout"><aside className="message-contacts"><header><b>Contacts</b><small>{channel==="email"?"Email follow-up queue":"Live text inbox"}</small></header>{leads.map(lead=>{const blocked=channel==="sms"?lead.smsOptOut:lead.emailOptOut;const allowed=channel==="sms"?lead.smsConsent:lead.emailConsent;return <button key={lead.id} className={selected?.id===lead.id?"active":""} onClick={()=>{setSelectedId(lead.id);setDraft("");setSubject("")}}><i>{lead.name.split(" ").map(value=>value[0]).slice(0,2).join("")}</i><span><b>{lead.name}</b><small>{channel==="email"?lead.email||"No email":lead.phone}</small></span>{blocked?<em className="blocked">BLOCKED</em>:allowed?<em>PERMITTED</em>:null}</button>})}{!leads.length&&<p>Import leads to start a conversation.</p>}</aside>
      <section className="message-thread">{selected?<><header><div><b>{selected.name}</b><small>{selected.product} · {channel==="email"?selected.email||"No email":selected.phone}</small></div><label><input type="checkbox" checked={Boolean(consent&&!optedOut)} disabled={Boolean(optedOut||selected.doNotCall||(channel==="email"&&!selected.email))} onChange={event=>channel==="sms"?onPatch(selected.id,{smsConsent:event.target.checked}):onPatch(selected.id,{emailConsent:event.target.checked})}/> {channel==="email"?"Email permission documented":"SMS consent on file"}</label></header><div className="message-history">{thread.map(message=><article key={message.id} className={message.direction==="inbound"||/inbound/i.test(message.direction)?"incoming":"outgoing"}>{message.subject&&<b className="message-subject">{message.subject}</b>}<p>{message.body}</p><small>{new Date(message.sentAt).toLocaleString()} · {message.status} · {message.provider}</small>{message.failureReason&&<strong className="message-failure">{message.failureReason}</strong>}</article>)}{!thread.length&&<div className="empty-thread"><b>No {channel} conversation yet</b><span>Generate a personal draft or choose a saved prompt below.</span></div>}</div><footer><MessageTemplateVault channel={channel} lead={selected} profile={profile} subject={subject} body={draft} onUse={(nextSubject,nextBody)=>{setSubject(nextSubject);setDraft(nextBody);setStatus("Template loaded and personalized")}} onProfileChange={onProfileChange}/>{aiMode&&<div className={`message-ai-mode ${aiMode}`}><b>{aiMode==="ai"?"Pacifica AI draft":"Pacifica Smart Fallback"}</b><span>Written from this contact’s product, source, stage, and notes</span></div>}{channel==="email"&&<input className="email-subject-input" value={subject} onChange={event=>setSubject(event.target.value)} placeholder="Email subject"/>}<textarea value={draft} onChange={event=>setDraft(event.target.value)} placeholder={optedOut?"This channel is blocked":`Write a ${channel==="email"?"personal email":"friendly text"}…`} disabled={Boolean(optedOut)}/>{!consent&&!optedOut&&!selected.doNotCall&&<p className="message-consent-warning">Document {channel==="email"?"email permission":"SMS consent"} above before sending. Drafting remains available.</p>}<div><button onClick={()=>void generateSelected()} disabled={loading||Boolean(optedOut)}>{loading?"Working…":"Draft with Pacifica AI"}</button><button className="send-message" onClick={()=>void sendSelected()} disabled={loading||!draft.trim()||Boolean(optedOut)||selected.doNotCall||(channel==="email"&&(!subject.trim()||!selected.email))}>Send via {channel==="email"?(emailStatus.provider==="none"?"email adapter":emailStatus.provider):twilioNumber||"SMS"}</button></div><small>{channel==="email"?"Email requires a verified sender domain, permission, business address, and unsubscribe handling.":"Marketing texts require documented consent. Pacifica blocks DNC records and recognized STOP replies."}</small></footer></>:<div className="empty-thread"><b>No contact selected</b></div>}</section></div>
  </div>;
}
