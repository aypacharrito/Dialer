...
import {blocksAiText,requiresPersonalText} from "../lib/ai-sms-recipients";
...
import {appendCommunication,cleanCommunications,type StoredCommunication} from "../lib/communications";
...
export default function MessagesCenter({workspaceId,profile,leads,onPatch,onProfileChange,initialLeadId,initialChannel,onOpenContact,onCloseLead}:{workspaceId:string;profile:WorkspaceProfile;leads:MessageLead[];onPatch:(id:number,patch:Partial<MessageLead>)=>void;onProfileChange:(profile:WorkspaceProfile)=>void;initialLeadId?:number|null;initialChannel?:Channel;onOpenContact:(id:number)=>void;onCloseLead:(id:number)=>void}){
  ...
  const latestMessages=useMemo(()=>{
    ...
    for(const lead of leads){
      const messages=channel==="sms"?smsMessages.filter(message=>digits(message.from)===digits(lead.phone)||digits(message.to)===digits(lead.phone)):(lead.communications||[]).filter(message=>message.channel==="email");
  ...
  const load=useCallback(async()=>{
    ...
    const [smsResult,emailResult,workspaceResult]=await Promise.allSettled([
      ...
      fetch("/api/crm/workspace",{cache:"no-store",credentials:"same-origin"}).then(async response=>({response,data:await response.json() as {leads?:MessageLead[]}})),
    ]);
    ...
    if(workspaceResult.status==="fulfilled"&&workspaceResult.value.response.ok){
      for(const remote of workspaceResult.value.data.leads||[]){
        const local=leadSnapshot.current.find(lead=>lead.id===remote.id);if(!local)continue;
        ...
        const communications=Array.from(merged.values()).sort((a,b)=>Date.parse(a.sentAt)-Date.parse(b.sentAt)).slice(-200);
  ...
  const thread=useMemo(()=>{
    if(!selected)return [];
    if(channel==="sms")return smsMessages.filter(message=>digits(message.from)===digits(selected.phone)||digits(message.to)===digits(selected.phone)).map(message=>({...message,channel:"sms" as const,subject:"",provider:"SMS"})).sort((a,b)=>new Date(a.sentAt).getTime()-new Date(b.sentAt).getTime());
  ...
  useEffect(()=>{const history=historyRef.current;if(history)history.scrollTop=history.scrollHeight},[selected?.id,channel,thread.length]);
  ...
  async function sendSms(lead:MessageLead,text:string){
    ...
    const response=await fetch("/api/twilio/messages",{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:lead.phone,body:text,permissionDocumented:lead.smsConsent===true})});const data=await response.json() as {message?:SmsMessage;error?:string};if(!response.ok||!data.message)throw new Error(data.error||"Text message could not be sent");setSmsMessages(old=>[...old,data.message!]);onPatch(lead.id,{lastSmsAt:new Date().toISOString()});deliveryTimers.current.forEach(window.clearTimeout);deliveryTimers.current=[1200,3000,6000,12000,20000,30000].map(delay=>window.setTimeout(()=>void load(),delay));
  }
