...
export default function LeadGrowthPanel({lead,teamMembers,onPatch,onGoogleCalendar,onDownloadCalendar}:Props){
  ...
  return <section className="lead-growth-panel quote-workspace" aria-label="Quote information, ownership, and scheduling">
    <header className="quote-workspace-header"><div><span>QUOTE WORKSPACE</span><b>Coverage-ready lead profile</b></div>{sourceEntries.length>0&&<em>{sourceEntries.length} SOURCE FIELDS</em>}</header>
    ...
    <footer><button onClick={onGoogleCalendar} disabled={!lead.followUp}>Google Calendar</button><button onClick={onDownloadCalendar} disabled={!lead.followUp}>Download .ics</button><button className="won" onClick={()=>onPatch({outcome:"Sold / Won",stage:"Closed"})}>Mark won</button></footer>
  </section>;
}
