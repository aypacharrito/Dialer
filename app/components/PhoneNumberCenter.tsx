...
export default function PhoneNumberCenter({currentWorkspaceId,onAssignmentChange}:{currentWorkspaceId:string;onAssignmentChange:()=>void}){
  ...
  return <section className="number-center">
    ...
    <div className="owned-number-list"><h3>Owned Twilio numbers</h3>{(data?.numbers||[]).map(number=>{const assignment=assignmentsByPhone.get(number.phoneNumber);return <article key={number.sid}><div><b>{formatted(number.phoneNumber)}</b><span>{assignment?`${assignment.workspaceName} · ${assignment.workspaceEmail}`:"Unassigned"}</span></div><div className="number-readiness"><em className={number.voiceReady?"ready":"attention"}>{number.voiceReady?"VOICE READY":"VOICE NEEDS CONFIG"}</em><em className={assignment?.smsStatus==="registered"?"ready":"attention"}>{assignment?.smsStatus==="registered"?"SMS REGISTERED":"A2P REQUIRED"}</em></div><button disabled={busy!==""||!data?.storageConfigured} onClick={()=>void assignOwned(number)}>{busy===number.sid?"Assigning…":assignment?.workspaceId===selectedWorkspace?"Assigned":"Assign"}</button></article>})}{data&&!data.numbers.length&&<p>No Twilio numbers are owned by the connected account yet.</p>}</div>
    <footer><b>Protected setup</b><span>Only the Pacifica platform owner can purchase or reassign numbers. Releasing numbers is intentionally handled in Twilio to prevent accidental loss. SMS remains blocked until that customer’s A2P Brand, Campaign, and Messaging Service are approved.</span></footer>
  </section>;
}
