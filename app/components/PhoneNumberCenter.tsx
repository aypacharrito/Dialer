...
type CenterData={configured:boolean;smsSendingEnabled:boolean;storageConfigured:boolean;voiceUrl:string;providerStatus:ProviderStatus;workspaces:Workspace[];numbers:OwnedNumber[];assignments:Assignment[]};
...
export default function PhoneNumberCenter({currentWorkspaceId,onAssignmentChange}:{currentWorkspaceId:string;onAssignmentChange:()=>void}){
  ...
  return <section className="number-center">
    ...
    {data&&!data.smsSendingEnabled&&data.assignments.some(item=>item.smsStatus==="registered")&&<div className="sms-owner-setup"><b>Registration recorded · sending paused</b><p>In Vercel → Settings → Environment Variables, set <code>TWILIO_A2P_APPROVED</code> to <code>true</code> for Production, then redeploy. Enable it only after Twilio approves the number’s campaign.</p></div>}
