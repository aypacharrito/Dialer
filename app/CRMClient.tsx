...
export default function Page({clerkEnabled=false,isOwner=false,isPlatformOwner=false,workspaceId="local",currentUserName="Pacifica user",currentUserEmail="",userRole="owner"}:{clerkEnabled?:boolean;isOwner?:boolean;isPlatformOwner?:boolean;workspaceId?:string;currentUserName?:string;currentUserEmail?:string;userRole?:string}){
  ...
  function finishCall(wasManual:boolean,leadId?:number,message="Call ended — save an outcome, then resume",outcome="Completed",errorCode?:string,attemptId=callAttemptRef.current){
    ...
    callDigits.clear();callRef.current=null;callStartGateRef.current.finish();setCurrentCallLeadId(null);setLoadedLeadId(null);setRecordingSid("");setIndex(0);setDialing(false);setConnected(false);setSeconds(0);elapsedRef.current=0;setMuted(false);setHeld(false);setDtmfDisplay("");setDtmfFeedback({message:"",error:false});setManualCall(false);setWorkspaceProfile(profile=>({...profile,liveCallSession:null}));
  ...
  return <main className="app-shell" onDragEnter={onFileDragEnter} onDragOver={onFileDragOver} onDragLeave={onFileDragLeave} onDrop={onFileDrop}>
    ...
    <section className="workspace">
      ...
      {dialing&&view!=="dialer"&&<ActiveCallBar name={manualCall?"Manual call":lead.name} number={manualCall?dialNumber:lead.phone} connected={connected} held={held} muted={muted} elapsed={fmt} queueRunning={autoDialing} onOpen={()=>setView("dialer")} onKeypad={openCallKeypad} onMute={toggleMute} onHold={toggleHold} onEnd={hangup} onPause={pauseQueue}/>}
      ...
      {view==="dialer"&&<div className={`dialer-view conversation-workspace ${connected||postCallLeadId?"has-conversation":"is-waiting"} ${keypadOpen?"has-keypad":""}`}>
        ...
        {keypadOpen&&<DialerKeypad connected={connected} dialing={dialing} phoneReady={phoneReady} number={dialNumber} sentDigits={dtmfDisplay} feedback={dtmfFeedback} onNumberChange={setDialNumber} onDigits={pressKey} onCall={callTypedNumber}/>}
        </div>
      </div>}
