...
export default function Page({clerkEnabled=false,isOwner=false}:{clerkEnabled?:boolean;isOwner?:boolean}){
  ...
  function start(){ if(postCallLeadId){setToast("Save the connected call result before continuing");return} if(!callableLeads.length){setView("leads");setToast(lineLeads.length?"No open contacts are eligible to dial":`Import ${activeLine==="life"?"Life":"Home & Auto"} leads first`);return} autoDialRef.current=true;setAutoDialing(true);void placeCall(lead.phone,false,lead) }
  ...
  async function syncLeadFeed(key=leadFeedKey,notify=true){
    ...
    try{
      ...
      const data=await response.json() as {error?:string;leads?:Array<{id:string;vendorId?:string;source?:string;name:string;phone:string;email:string;city:string;product:string;line:LeadLine;disposition:string;notes:string;cost:number;createdAt:string}>};
  ...
  const nav:[View,string,string][]=[["dialer","Dialer","dial"],["leads","CRM contacts","users"],["messages","Messages","chat"],["ai","Pacifica AI","spark"],["quotes","Quote center","shield"],["campaigns","Pipeline","list"],["activity","Reports","chart"],["billing","Plans & billing","list"]];
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {view==="quotes"&&<QuoteCenter locked leads={lineLeads.map(({id,name,phone,email,city})=>({id,name,phone,email,city}))} onOpenContact={id=>setSelectedLead(id)}/>}
      ...
      {view==="billing"&&<div className="page-view billing-view"><div className="page-title"><div><span className="eyebrow">PACIFICA SUBSCRIPTIONS</span><h1>Choose the workspace that fits your agency.</h1><p>Every plan uses secure Stripe-hosted checkout. Assigned calling numbers are provisioned by the Pacifica team after verification.</p></div></div><div className="pricing-grid">
        <article><span>SOLO</span><h2>${pacificaPlans.solo.monthlyPrice}<small>/month</small></h2><p>{pacificaPlans.solo.description}</p><ul><li>{pacificaPlans.solo.seats}</li><li>{pacificaPlans.solo.numbers}</li><li>Life and Home & Auto CRMs</li><li>Sequential auto dialer</li><li>Pacifica ClearVoice noise suppression</li><li>Pacifica AI and reports</li><li>Quote Center coming soon</li></ul><button disabled={Boolean(checkoutPlan)} onClick={()=>void subscribe("solo")}>{checkoutPlan==="solo"?"Opening secure checkout…":"Start Solo"}</button></article>
        <article className="featured"><em>MOST POPULAR</em><span>TEAM</span><h2>${pacificaPlans.team.monthlyPrice}<small>/month</small></h2><p>{pacificaPlans.team.description}</p><ul><li>{pacificaPlans.team.seats}</li><li>{pacificaPlans.team.numbers}</li><li>Separate Life and Home & Auto queues</li><li>Compliance controls and call reporting</li><li>Priority onboarding</li><li>Quote Center coming soon</li></ul><button disabled={Boolean(checkoutPlan)} onClick={()=>void subscribe("team")}>{checkoutPlan==="team"?"Opening secure checkout…":"Start Team"}</button></article>
        <article><span>AGENCY</span><h2>${pacificaPlans.agency.monthlyPrice}<small>/month</small></h2><p>{pacificaPlans.agency.description}</p><ul><li>{pacificaPlans.agency.seats}</li><li>{pacificaPlans.agency.numbers}</li><li>Agency-level campaigns and reporting</li><li>Number reputation monitoring</li><li>White-glove setup</li><li>Quote Center coming soon</li></ul><button disabled={Boolean(checkoutPlan)} onClick={()=>void subscribe("agency")}>{checkoutPlan==="agency"?"Opening secure checkout…":"Start Agency"}</button></article>
