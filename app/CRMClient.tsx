...
export default function Page({clerkEnabled=false,isOwner=false,workspaceId="local"}:{clerkEnabled?:boolean;isOwner?:boolean;workspaceId?:string}){
  ...
  function scheduleNextAuto(completedLeadId?:number){
    ...
    if(!queue.length||nextPosition>=queue.length){stopAutoDial(`${queueLabel(activeLineRef.current,workspaceProfile.mode)} queue completed`);return}
  ...
  function acceptIncoming(){
    ...
    call.on("accept",()=>{if(currentLogRef.current)currentLogRef.current.connectedAt=Date.now();setConnected(true);setSeconds(0);setPhoneStatus(`Live inbound call${matched?` · ${matched.name}`:""}`);if(matched){activeLineRef.current=matched.line;setActiveLine(matched.line);const queue=leadsRef.current.filter(item=>item.line===matched.line&&item.stage!=="Closed"&&!item.doNotCall);setIndex(Math.max(0,queue.findIndex(item=>item.id===matched.id)));setView("dialer");updateLead(matched.id,{lastContact:new Date().toLocaleString()})}});
