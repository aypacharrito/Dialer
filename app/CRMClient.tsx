...
type NewLeadDraft={name:string;phone:string;email:string;city:string;address:string;state:string;zip:string;product:string;source:string;leadCost:string;dateOfBirth:string;licenseNumber:string;licenseState:string;licenseExpiration:string;policyNumber:string;policyEffectiveDate:string;policyExpirationDate:string;vin:string;vehicle:string;documentType:string;documentFields:Record<string,string>};
const emptyNewLead:NewLeadDraft={name:"",phone:"",email:"",city:"",address:"",state:"",zip:"",product:"",source:"Manual",leadCost:"",dateOfBirth:"",licenseNumber:"",licenseState:"",licenseExpiration:"",policyNumber:"",policyEffectiveDate:"",policyExpirationDate:"",vin:"",vehicle:"",documentType:"",documentFields:{}};
...
async function documentImageDataUrl(file:File){
  if(typeof createImageBitmap!=="function")return new Promise<string>((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result||""));reader.onerror=()=>reject(new Error("The image could not be opened"));reader.readAsDataURL(file)});
  const bitmap=await createImageBitmap(file);const largest=Math.max(bitmap.width,bitmap.height);const scale=Math.min(1,1800/largest);const canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));const context=canvas.getContext("2d");if(!context){bitmap.close();throw new Error("The image could not be prepared")};context.drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close();return canvas.toDataURL("image/jpeg",.84);
}
...
export default function Page({clerkEnabled=false,isOwner=false,isPlatformOwner=false,workspaceId="local",currentUserName="Pacifica user",currentUserEmail="",userRole="owner"}:{clerkEnabled?:boolean;isOwner?:boolean;isPlatformOwner?:boolean;workspaceId?:string;currentUserName?:string;currentUserEmail?:string;userRole?:string}){
  ...
  const nav:[View,string,string][]=[["today","Today","spark"],["dialer","Dialer","dial"],["leads","Contacts","users"],["messages","Messages","chat"],["campaigns","Pipeline","list"],["clients","Clients","shield"],["revenue","Revenue","chart"],["activity","Reports","chart"],["ai","Pacifica AI","spark"],["quotes","Industry Tools","shield"],["billing","Plans & Billing","list"]];
  ...
  function createLead(){const phone=newLead.phone.trim();const name=newLead.name.trim();if(!name||phone.replace(/\D/g,"").length<7){setToast("Add a name and complete phone number");return}const createdAt=new Date().toISOString();const item:Lead={id:Date.now(),name,phone,email:newLead.email.trim(),city:newLead.city.trim()||"No city",address:newLead.address.trim(),state:newLead.state.trim(),zip:newLead.zip.trim(),status:"Ready",stage:"New lead",outcome:"Not contacted",notes:"",followUp:"",doNotCall:false,lastContact:"Never",line:activeLine,source:newLead.source.trim()||"Manual",leadCost:Number(newLead.leadCost)||0,product:newLead.product.trim()||"Service inquiry",sourceDisposition:"New",importedAt:createdAt,received:createdAt,smsConsent:false,smsOptOut:false,emailConsent:false,emailOptOut:false,communications:[],automationEnabled:true,dateOfBirth:newLead.dateOfBirth,licenseNumber:newLead.licenseNumber,licenseState:newLead.licenseState,licenseExpiration:newLead.licenseExpiration,policyNumber:newLead.policyNumber,policyEffectiveDate:newLead.policyEffectiveDate,policyExpirationDate:newLead.policyExpirationDate,renewalDate:newLead.policyExpirationDate,vin:newLead.vin,vehicle:newLead.vehicle,importedFields:newLead.documentFields};setLeads(old=>[item,...old]);setNewLead(emptyNewLead);setShowNewLead(false);setSelectedLead(item.id);setView("leads");setToast(`${name} added to ${queueLabel(activeLine,workspaceProfile.mode)}`)}
  async function scanDocument(file?:File){
    if(!file)return;if(!["image/jpeg","image/png","image/webp"].includes(file.type)){setToast("Use a JPEG, PNG, or WebP photo");return}if(file.size>15*1024*1024){setToast("Use an image smaller than 15 MB");return}
    setScanBusy(true);setToast("Reading document…");
    try{const response=await fetch("/api/ai/document-lead",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image:await documentImageDataUrl(file),fileName:file.name})});const data=await response.json() as {extraction?:DocumentLeadExtraction;error?:string};if(!response.ok||!data.extraction)throw new Error(data.error||"Document scan failed");const extraction=data.extraction;setNewLead({...emptyNewLead,name:documentLeadName(extraction),phone:extraction.phone,email:extraction.email,city:extraction.city,address:extraction.address,state:extraction.state,zip:extraction.zip,product:extraction.product||(/insurance|policy/i.test(extraction.documentType)?"Insurance quote":"Service inquiry"),source:"Document scan",dateOfBirth:extraction.dateOfBirth,licenseNumber:extraction.licenseNumber,licenseState:extraction.licenseState,licenseExpiration:extraction.licenseExpiration,policyNumber:extraction.policyNumber,policyEffectiveDate:extraction.policyEffectiveDate,policyExpirationDate:extraction.policyExpirationDate,vin:extraction.vin,vehicle:[extraction.vehicleYear,extraction.vehicleMake,extraction.vehicleModel].filter(Boolean).join(" "),documentType:extraction.documentType,documentFields:documentLeadImportedFields(extraction)});setShowNewLead(true);setToast("Document read · review the fields and add a phone number")}
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {view==="leads"&&<div className="page-view crm-view"><header className="module-bar"><span className="eyebrow">{queueLabel(activeLine,workspaceProfile.mode).toUpperCase()} CONTACTS</span><button className="primary" onClick={()=>inputRef.current?.click()}><Icon name="upload"/> Import</button></header>{importReport&&<div className="import-hint"><span>LAST IMPORT</span><p>{importReport}</p></div>}<div className="crm-summary five"><article><span>CONTACTS</span><b>{lineLeads.length}</b></article><article><span>LEAD SPEND</span><b>${lineLeads.reduce((total,leadItem)=>total+leadItem.leadCost,0).toFixed(0)}</b></article><article><span>FOLLOW-UPS</span><b>{dueLeadCount}</b></article><article><span>APPOINTMENTS</span><b>{lineLeads.filter(l=>l.stage==="Appointment").length}</b></article><article><span>REVENUE</span><b>${lineLeads.reduce((total,item)=>total+(item.closedRevenue||0),0).toFixed(0)}</b></article></div><div className="crm-tools"><label><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search all contact and imported fields"/></label><select aria-label="Filter by source" value={sourceFilter} onChange={e=>setSourceFilter(e.target.value)}>{sourceOptions.map(source=><option key={source}>{source}</option>)}</select><select aria-label="Filter by owner" value={ownerFilter} onChange={e=>setOwnerFilter(e.target.value)}>{ownerOptions.map(owner=><option key={owner}>{owner}</option>)}</select><select aria-label="Filter by pipeline stage" value={stageFilter} onChange={e=>setStageFilter(e.target.value)}><option>All stages</option><option>New lead</option><option>Follow-up</option><option>Appointment</option><option>Closed</option></select><select aria-label="Sort contacts" value={leadSort} onChange={e=>setLeadSort(e.target.value)}><option value="Next best">Recommended order</option><option>Newest</option><option>Follow-up due</option><option>Recently attempted</option><option>Name</option></select></div><div className="table-card crm-table smart-table"><div className="table-head"><span>CONTACT & PRIORITY</span><span>SOURCE & OWNER</span><span>STAGE</span><span>LAST RESULT</span><span>NEXT FOLLOW-UP</span></div>{filteredLeads.map(l=>{const priority=leadPriority(l,priorityNow);const nextAction=l.followUp||l.automationNextAt;return <button className="table-row" key={l.id} onClick={()=>setSelectedLead(l.id)}><span><i>{l.name.split(" ").map(x=>x[0]).slice(0,2).join("")}</i><span><b>{l.name}</b><small>{l.phone||"No phone"} · {l.email||"No email"}</small><small className="priority-summary"><em className={`priority-pill ${priority.level.toLowerCase()}`}>{priority.level}</em>{priority.reason}</small></span></span><span className="lead-source"><b>{l.source}</b><small>{l.assignedTo||"Unassigned"}{l.leadCost?` · $${l.leadCost.toFixed(2)}`:""}</small></span><span><em className={`stage ${l.stage.toLowerCase().replace(" ","-")}`}>{l.stage}</em></span><span>{l.outcome}<small>{l.attempts||0} attempt{l.attempts===1?"":"s"}</small></span><span>{nextAction?new Date(nextAction).toLocaleString():"—"}{l.doNotCall&&<strong className="dnc">DNC</strong>}</span></button>})}{!filteredLeads.length&&<div className="empty-state">No matching contacts</div>}</div></div>}
    ...
    {showNewLead&&<div className="new-lead-backdrop" onClick={()=>setShowNewLead(false)}><form className="new-lead-modal" onSubmit={event=>{event.preventDefault();createLead()}} onClick={event=>event.stopPropagation()}>
      ...
      <div className="scan-lead-action"><button type="button" onClick={()=>scanInputRef.current?.click()}><Icon name="camera"/> {newLead.documentType?"Scan another":"Scan a license or policy"}</button>{newLead.documentType&&<span>{newLead.documentType} · verify before saving</span>}</div>
