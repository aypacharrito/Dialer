...
import {desktopNotifications,type DesktopMessage} from "./lib/desktop-notifications";
import CalendarAlerts from "./components/CalendarAlerts";

import {createCallScreening} from "./lib/call-screening";
import type {ContactCallDetection} from "./lib/call-detection";
import {mergeIncomingContacts,contactMatches} from "./lib/contact-sync";
...
import type {QuoteRequestEvidence} from "./lib/quote-intake";
...
import type {MinerMode} from "./components/MinerPanel";
...
import { crmFieldsForDisposition, dateValue, isDialerEligibleLead, leadCreatedAt, leadLineForProduct, leadPriority, rankLeads, rankDialerLeads, refreshDialerRun, sourceDispositionForOutcome, suggestedRetryAt, validReceivedDate } from "./lib/lead-priority";
...
import {openCallMicrophone,configureCallAudio,callSetupMessage} from "./lib/call-audio-setup";
import { readAudioPreferences,saveAudioPreferences } from "./audio-preferences";
...
const IndustryTools=dynamic(()=>import("./components/IndustryTools"),{loading:()=> <p role="status">Loading…</p>});

const OpportunityDesk=dynamic(()=>import("./components/OpportunityDesk"),{loading:()=> <p role="status">Loading…</p>});

const ClientPortfolio=dynamic(()=>import("./components/ClientPortfolio"),{loading:()=> <p role="status">Loading…</p>});

const MinerPanel=dynamic(()=>import("./components/MinerPanel"),{loading:()=> <p role="status">Loading…</p>});

type LeadLine = "life" | "home-auto";
type DialerScope="crm"|"miner-personal-auto"|"miner-home"|"miner-commercial";
type Lead = ContactCallDetection & { quoteRequests?:QuoteRequestEvidence[];quoteDetailsUpdatedAt?:string; deletedAt?:string;deletionUpdatedAt?:string;id:number; name:string; phone:string; city:string; status:string; email:string; stage:string; outcome:string; notes:string; followUp:string; followUpUtc?:string; doNotCall:boolean; lastContact:string; line:LeadLine; queueOverride?:boolean; source:string; leadCost:number; product:string; sourceDisposition:string; importedAt:string; vendorId?:string; sourceSyncStatus?:string; providerUpdatedAt?:string; address?:string; state?:string; zip?:string; territory?:string; brand?:string; profileName?:string; received?:string; returnStatus?:string; employeeCount?:string; searchPro?:string; extraFields?:Record<string,string>; csvFileName?:string; csvUpdatedAt?:string; importedFields?:Record<string,string>; smsConsent?:boolean; smsOptOut?:boolean; lastSmsAt?:string; emailConsent?:boolean; emailOptOut?:boolean; lastEmailAt?:string; communications?:StoredCommunication[]; attempts?:number; lastAttemptAt?:string; lastConnectedAt?:string; priorityOverride?:"auto"|"high"|"low"; assignedTo?:string; estimatedValue?:number; closedRevenue?:number; closedAt?:string; automationEnabled?:boolean; automationSequenceId?:string; automationStep?:number; automationNextAt?:string; automationStatus?:string; automationDeliveryFailures?:number; automationLastError?:string; automationDeadLetterAt?:string; automationUpdatedAt?:string; lastInboundAt?:string; clientStatus?:"active"|"inactive"; dateOfBirth?:string; policyNumber?:string; policyEffectiveDate?:string; policyExpirationDate?:string; renewalDate?:string; policyPremium?:number; policyTermMonths?:number; clientReminderKeys?:string[]; licenseNumber?:string; licenseState?:string; licenseExpiration?:string; vin?:string; vehicle?:string };
type View = "office" | "opportunities" | "today" | "dialer" | "leads" | "miner" | "messages" | "ai" | "quotes" | "clients" | "activity" | "settings";
type SettingsSection = "accounts" | "billing" | "workspace" | "team" | "phone" | "integrations" | "system";
...
function Icon({name}:{name:string}) {
  const paths:Record<string,React.ReactNode> = {
    ...
    pickaxe:<><path d="m4 20 10-10M3 7c5-5 12-4 17 3-6-3-9-3-12 0L3 7Z"/></>,
    robot:<><rect x="4" y="7" width="16" height="14" rx="4"/><path d="M12 3v4M1 12v5M23 12v5M9 17h6"/><circle cx="9" cy="12" r="1"/><circle cx="15" cy="12" r="1"/></>,
...
export default function Page({clerkEnabled=false,isOwner=false,isPlatformOwner=false,workspaceId="local",currentUserName="Pacifica user",currentUserEmail="",userRole="owner"}:{clerkEnabled?:boolean;isOwner?:boolean;isPlatformOwner?:boolean;workspaceId?:string;currentUserName?:string;currentUserEmail?:string;userRole?:string}){
  ...
  const [quoteRequestLeadId,setQuoteRequestLeadId]=useState<number|null>(null);
  ...
  const [aiActivity,setAiActivity]=useState<"idle"|"working"|"sending"|"ready">("idle");
  const [messageTarget,setMessageTarget]=useState<{leadId:number;channel:"sms"|"email"}|null>(null);
  ...
  const [dtmfFeedback,setDtmfFeedback]=useState({message:"",error:false});
  const [callDigits]=useState(createCallDigitQueue);
  useEffect(()=>()=>callDigits.clear(),[callDigits]);
  const [keypadOpen,setKeypadOpen]=useState(false);
  useEffect(()=>{if(keypadOpen&&view==="dialer")document.getElementById("manual-dial-number")?.focus()},[keypadOpen,view]);
  ...
  const audioDeviceChangeTimerRef=useRef<number|undefined>(undefined);
  ...
  const workspaceSaveQueue=useMemo(()=>({workspaceId,queue:createWorkspaceSaveQueue<string>(async body=>{
    const response=await fetch("/api/crm/workspace",{method:"PUT",headers:{"Content-Type":"application/json"},body});
    if(!response.ok)throw new Error("Cloud save failed");
  })}),[workspaceId]);
  useEffect(()=>()=>workspaceSaveQueue.queue.cancelPending(),[workspaceSaveQueue]);
  ...
  const scopedDialerLeads=useMemo(()=>dialerScope==="miner-personal-auto"?minerLeads.filter(item=>/Personal Auto/i.test(item.source)):dialerScope==="miner-home"?minerLeads.filter(item=>/Pacifica Miner\s*·\s*Home/i.test(item.source)):dialerScope==="miner-commercial"?minerLeads.filter(item=>/Commercial/i.test(item.source)):lineLeads,[dialerScope,minerLeads,lineLeads]);
  ...
  useEffect(()=>{
    ...
    let saved=false;
    const persistLocal=()=>{if(saved)return;try{localStorage.setItem(`pacifica:${workspaceId}:leads`,JSON.stringify(allLeads));localStorage.setItem(`pacifica:${workspaceId}:call-logs`,JSON.stringify(callLogs.slice(0,500)));localStorage.setItem(`pacifica:${workspaceId}:profile`,JSON.stringify(workspaceProfile));saved=true}catch{}};
    if(!clerkEnabled){persistLocal();queueMicrotask(()=>setWorkspaceSyncStatus("Saved in this browser"));return}
    queueMicrotask(()=>setWorkspaceSyncStatus("Saving changes…"));
  ...
  useEffect(()=>{if(!workspaceHydrated||!clerkEnabled)return;let canceled=false;let refreshing=false;const seenKey=`pacifica:${workspaceId}:messages-seen`;const refresh=()=>{if(refreshing)return;refreshing=true;void fetch("/api/crm/workspace",{cache:"no-store"}).then(response=>{if(!response.ok)throw new Error("Workspace refresh failed");return response.json()}).then(data=>{if(canceled)return;if(Array.isArray(data.callLogs))setCallLogs(current=>mergeRecordingUpdates(current,data.callLogs as CallLog[]));const remoteLeads=normalizeSavedLeads(data.leads);setLeads(current=>mergeIncomingContacts(current,remoteLeads));let seen=0;try{seen=Number(localStorage.getItem(seenKey))||0}catch{}const inbound=remoteLeads.flatMap(item=>(item.communications||[]).filter(comm=>String(comm.direction||"").toLowerCase().includes("in")).map(comm=>({lead:item,time:new Date(String(comm.sentAt||"")).getTime()||0,body:String(comm.body||comm.subject||"New message"),channel:comm.channel==="email"?"email" as const:"sms" as const,id:String(comm.providerId||comm.id)}))).filter(item=>item.time>seen).sort((a,b)=>b.time-a.time);setMessageUnreadCount(inbound.length);const latest=inbound[0];if(latest&&latest.time>latestInboundRef.current){if(latestInboundRef.current){for(const item of inbound.filter(x=>x.time>latestInboundRef.current).slice(0,10).reverse()){const desktop=desktopNotifications();if(desktop?.notifyMessage)desktop.notifyMessage({id:item.id,leadId:item.lead.id,name:item.lead.name,body:item.body,channel:item.channel,theme:document.documentElement.dataset.theme||"light"});else if(typeof Notification!=="undefined"&&Notification.permission==="granted")new Notification(`New message from ${item.lead.name}`,{body:item.body,icon:"/pacifica-icon-192.png"})}}latestInboundRef.current=latest.time}if(!latestInboundRef.current)latestInboundRef.current=Date.now()}).catch(()=>undefined).finally(()=>{refreshing=false})};const initial=window.setTimeout(refresh,1500);const timer=window.setInterval(refresh,5000);return()=>{canceled=true;window.clearTimeout(initial);window.clearInterval(timer)}},[clerkEnabled,workspaceHydrated,workspaceId]);
  function openView(id:View){if(id==="dialer"){dialerScopeRef.current="crm";setDialerScope("crm");minerRunIdsRef.current=[];setMinerRunIds([])}if(id==="messages"){try{localStorage.setItem(`pacifica:${workspaceId}:messages-seen`,String(Date.now()))}catch{}setMessageUnreadCount(0);if(!desktopNotifications()?.isDesktop&&typeof Notification!=="undefined"&&Notification.permission==="default")void Notification.requestPermission()}setGrowthLeadId(null);setSelectedLead(null);setView(id)}
    ...
    const initialization=(async()=>{
      ...
      if(!device){
        ...
        device.on("incoming",call=>{
          ...
          try{
            if(!desktopNotifications()?.supportsDesktopWrapUp&&typeof Notification!=="undefined"&&Notification.permission==="granted"){
        ...
        device.audio?.on("deviceChange",()=>{
          ...
          audioDeviceChangeTimerRef.current=window.setTimeout(()=>{
            audioDeviceChangeTimerRef.current=undefined;
            const active=deviceRef.current;if(!active)return;
            const preferences=readAudioPreferences();
            void configureCallAudio(active.audio,preferences).then(async recovered=>{
              if(Object.keys(recovered).length){saveAudioPreferences(recovered);setToast("Audio source changed · Pacifica switched the missing device to Windows default")}
              await configureClearVoice(active);
              setPhoneStatus(callRef.current?"Audio source changed · call audio re-routed":"Audio source changed · phone ready");
            }).catch(error=>{
              const message=callSetupMessage(error);
              setPhoneStatus(message);setToast("Audio source changed · open Calling settings if you cannot hear the call");
            });
          },400);
        });
  ...
  function scopeForLead(item:Lead):DialerScope{return /Pacifica Miner\s*·\s*Personal Auto/i.test(item.source)?"miner-personal-auto":/Pacifica Miner\s*·\s*Home/i.test(item.source)?"miner-home":/Pacifica Miner\s*·\s*Commercial/i.test(item.source)?"miner-commercial":"crm"}
  function dialerQueueLabel(scope=dialerScopeRef.current){return scope==="miner-personal-auto"?"Miner · Personal Auto":scope==="miner-home"?"Miner · Homeowners":scope==="miner-commercial"?"Miner · Commercial":queueLabel(activeLineRef.current,workspaceProfile.mode)}
  ...
  function remainingDialerRunLeads(line:LeadLine){
    ...
    if(remaining.map(item=>item.id).join(",")!==run.ids.join(",")){
      const removed=run.ids.filter(id=>!eligibleById.has(id)).length;
      const added=remaining.filter(item=>!run.ids.includes(item.id)).length;
      const next=remaining.length?{...run,ids:remaining.map(item=>item.id),total:run.total+added,completed:Math.min(run.total,run.completed+removed),updatedAt:new Date().toISOString()}:null;
  ...
  function choosePostCallOutcome(outcome:string){
    ...
    setPostCallDraft(draft=>selectPostCallOutcome(draft,postCallLead.source,outcome,new Date(),postCallLead.stage==="Closed"&&draft.crmStage==="Closed"));
  }
  async function syncLeadDisposition(completedLead:Lead,patch:{sourceDisposition:string;stage:string;outcome:string;followUp:string;notes:string},callEnded=false){
    if(/smart[\s_-]*financial/i.test(completedLead.source)&&!callEnded)return completedLead.sourceSyncStatus||"Saved in Pacifica · SmartFinancial updates after an outbound call";
    try{
      const response=await fetch("/api/integrations/dispositions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:callEnded?"call-ended":"contact-saved",source:completedLead.source,vendorId:completedLead.vendorId,leadName:completedLead.name,phone:completedLead.phone,disposition:patch.sourceDisposition,crmStage:patch.stage,crmOutcome:patch.outcome,appointmentAt:patch.followUp,notes:patch.notes})});
  ...
  async function savePostCall(next:"queue"|"again"="queue",desktopDraft?:typeof postCallDraft){
    ...
    if(next==="again"){
      ...
      if(nextCallTimerRef.current)window.clearTimeout(nextCallTimerRef.current);
      setView("dialer");void placeCall(completedLead.phone,false,{...completedLead,...patch});
    }
    else if(resume){autoDialRef.current=true;setAutoDialing(true);scheduleNextAuto()}
  ...
  function finishCall(wasManual:boolean,leadId?:number,message="Call ended — save an outcome, then resume",outcome="Completed",errorCode?:string,attemptId=callAttemptRef.current,automaticSkip=false){
    ...
    // Fast Skip may use the historical Completed -> No answer fallback.
    // Listen Through never guesses: only confirmed No answer auto-advances.
    if(!wasConnected&&outcome==="Completed"&&!listenThroughMode)outcome="No answer";
    ...
    if(attemptWasEstablished&&leadId&&endedLog?.campaign!=="Inbound"){
      const attemptedLead=leadsRef.current.find(item=>item.id===leadId);
      if(attemptedLead&&/smart[\s_-]*financial/i.test(attemptedLead.source)){
        void syncLeadDisposition(attemptedLead,{sourceDisposition:"Attempted Contact",stage:attemptedLead.stage,outcome:attemptedLead.outcome,followUp:attemptedLead.followUp,notes:attemptedLead.notes},true);
      }
    }
    ...
    if(automaticSkip&&leadId&&shouldResume){
      ...
      if(item){
        const patch:Partial<Lead>={...(attemptWasEstablished?{}:{attempts:(item.attempts||0)+1,lastAttemptAt:new Date().toISOString()}),lastCallResult:outcome,lastContact:`${outcome} · ${new Date().toLocaleString()}`};
        if(!item.deletedAt&&!item.lastInboundAt&&["New lead","Follow-up","Open"].includes(item.stage)&&["Not contacted","No answer","Voicemail"].includes(item.outcome)&&["No answer","Voicemail"].includes(outcome))patch.outcome=outcome;
        updateLead(leadId,patch);completeDialerRunLead(leadId,item.line);
      }
      setPhoneStatus(`${outcome} · continuing`);scheduleNextAuto();return;
    }
  ...
  async function placeCall(number:string,wasManual:boolean,queuedLead:Lead=lead){
    ...
    try{
      ...
      if(Object.keys(recovered).length){saveAudioPreferences(recovered);setToast("Audio device changed · using your current default")}
      const clearVoiceActive=await configureClearVoice(device);
      if(attemptId!==callAttemptRef.current||advancingRef.current)return;
      let signalingExpired=false;let signalingTimer:number|undefined;
      const connectPromise=device.connect({params:{To:number,RouteToken:voiceRouteTokenRef.current,RingTimeout:String(workspaceProfile.dialerRingTimeoutSeconds)}}).then(call=>{if(signalingExpired||attemptId!==callAttemptRef.current||advancingRef.current)call.disconnect();return call});
      ...
      const detail=callSetupMessage(error);
  ...
  function startMinerDialer(mode:MinerMode){
    ...
    const scope:DialerScope=mode==="personal-auto"?"miner-personal-auto":mode==="home"?"miner-home":"miner-commercial";setMinerMode(mode);dialerScopeRef.current=scope;setDialerScope(scope);setLoadedLeadId(null);minerRunIdsRef.current=[];setMinerRunIds([]);
    const queue=createDialerRun(activeLineRef.current);if(!queue.length){setToast(`No ${mode==="personal-auto"?"personal-auto":mode==="home"?"homeowner":"commercial"} Miner prospects are ready to dial`);return}
  ...
  function callTypedNumber(){
    if(postCallLeadId){setToast("Save the call result or choose Call again first");return}
  ...
  async function togglePhoneAvailability(){
    if(!desktopNotifications()?.isDesktop&&typeof Notification!=="undefined"&&Notification.permission==="default")void Notification.requestPermission().catch(()=>undefined);
    try{
      const device=await ensureDevice();
  ...
  function acceptIncoming(){
    ...
    if(currentLogRef.current?.campaign!=="Inbound"&&establishedAttemptRef.current===callAttemptRef.current){
      const attemptedLead=leadsRef.current.find(item=>item.id===currentCallLeadId);
      if(attemptedLead&&/smart[\s_-]*financial/i.test(attemptedLead.source))void syncLeadDisposition(attemptedLead,{sourceDisposition:"Attempted Contact",stage:attemptedLead.stage,outcome:attemptedLead.outcome,followUp:attemptedLead.followUp,notes:attemptedLead.notes},true);
    }
  ...
  function pressKey(digits:string){
    if(dialing||callRef.current){
      callDigits.send(callRef.current,digits,{
        onSent:digit=>{setDtmfDisplay(value=>(value+digit).slice(-32));setDtmfFeedback({message:`Sent ${digit} to this call`,error:false})},
        onError:message=>setDtmfFeedback({message,error:true}),
      });return;
    }
    if(!/^[0-9*#]+$/.test(digits))return;
    for(const digit of digits)void playDialTone(digit);
    setDialNumber(value=>value+digits);setDtmfFeedback({message:"",error:false});
  }
  function openCallKeypad(){setView("dialer");setKeypadOpen(true)}
  ...
  useEffect(()=>{
    ...
    window.addEventListener("pacifica:inbound-synced",sync);window.addEventListener("pacifica:open-contact",open);
    return()=>{window.removeEventListener("pacifica:inbound-synced",sync);window.removeEventListener("pacifica:open-contact",open)};
  },[workspaceHydrated,workspaceId]);
  ...
  function importFile(file?: File, minerType?: MinerMode) {
    ...
    reader.onload = () => {
      ...
      const detectedSource=minerType?`Pacifica Miner · ${minerType==="personal-auto"?"Personal Auto":minerType==="home"?"Home":"Commercial"}`:header.includes("searchpro")&&header.includes("profilename")&&header.includes("territory")?"SmartFinancial":fileSource;
      ...
      rows.slice(hasHeader ? 1 : 0).forEach((row, rowIndex) => {
        ...
        const line: LeadLine = workspaceProfile.mode==="insurance"?(minerType?"home-auto":leadLineForProduct(product,activeLine)):/^(a|high|hot|priority)/.test(importedPriority)?"life":activeLine;
        ...
        parsed.push({
          ...
          product: product || (minerType==="personal-auto"?"Auto insurance prospect":minerType==="home"?"Home insurance prospect":minerType==="commercial"?"Commercial insurance prospect":"Service inquiry"),
  ...
  useEffect(()=>{
    ...
    if(!desktop.supportsDesktopWrapUp){desktop.setCallState({active:false});return;}
    const desktopIncoming=incomingNumber?findDialedContact(leads,incomingNumber):undefined;
  ...
  useEffect(()=>{
    ...
    return desktop.onCallAction(action=>{
      if(action==="answer-incoming"){acceptIncoming();return}
      if(action==="decline-incoming"){rejectIncoming();return}
  ...
  const handleDesktopMessage=useEffectEvent((message:DesktopMessage)=>{const contact=leads.find(item=>item.id===message.leadId);if(contact)openLeadMessage(contact,message.channel)});
  useEffect(()=>desktopNotifications()?.onMessageAction?.(message=>handleDesktopMessage(message)),[]);

  const nav:[View,string,string][]=([["today","Today","spark"],["dialer","Dialer","dial"],["leads","Contacts","users"],["messages","Messages","chat"],["office","Calendar","calendar"],...(workspaceProfile.mode==="insurance"?[["opportunities","Opportunities","bulb"],["miner","Miner","pickaxe"]]:[]),["clients","Clients","shield"],["activity","Reports","chart"],["ai","Pacifica AI","robot"],["quotes","Industry Tools","shield"]] as [View,string,string][]);
  ...
  const sourceOptions=useMemo(()=>["All sources",...Array.from(new Set(lineLeads.map(item=>item.source)))],[lineLeads]);
  const ownerOptions=useMemo(()=>["All owners","Unassigned",...Array.from(new Set([...workspaceProfile.teamMembers,...lineLeads.map(item=>item.assignedTo||"")].filter(Boolean)))],[workspaceProfile.teamMembers,lineLeads]);
  const matchingLeads=useMemo(()=>lineLeads.filter(l=>(stageFilter==="All stages"||l.stage===stageFilter)&&(sourceFilter==="All sources"||l.source===sourceFilter)&&(ownerFilter==="All owners"||(ownerFilter==="Unassigned"?!l.assignedTo:l.assignedTo===ownerFilter))&&contactMatches(l,deferredSearch)),[lineLeads,stageFilter,sourceFilter,ownerFilter,deferredSearch]);
  const filteredLeads=useMemo(()=>leadSort==="Next best"?rankLeads(matchingLeads,priorityNow):matchingLeads.toSorted((left,right)=>leadSort==="Newest"?leadCreatedAt(right)-leadCreatedAt(left):leadSort==="Follow-up due"?(Number.isFinite(dateValue(left.followUp))?dateValue(left.followUp):Number.POSITIVE_INFINITY)-(Number.isFinite(dateValue(right.followUp))?dateValue(right.followUp):Number.POSITIVE_INFINITY):leadSort==="Recently attempted"?(Number.isFinite(dateValue(right.lastAttemptAt))?dateValue(right.lastAttemptAt):0)-(Number.isFinite(dateValue(left.lastAttemptAt))?dateValue(left.lastAttemptAt):0):left.name.localeCompare(right.name)),[matchingLeads,leadSort,priorityNow]);
  const contactRows=useMemo(()=>filteredLeads.map(l=>{const priority=leadPriority(l,priorityNow);const nextAction=l.followUp||l.automationNextAt;return <button className="table-row" key={l.id} onClick={()=>setSelectedLead(l.id)}><span><i>{l.name.split(" ").map(x=>x[0]).slice(0,2).join("")}</i><span><span className="contact-name-line"><b>{l.name}</b><em className={`priority-pill ${priority.level.toLowerCase()}`}>{priority.level}</em></span><small>{l.phone||"No phone"} · {l.email||"No email"}</small><small className="priority-summary">{priority.reason}</small></span></span><span className="lead-source"><b>{l.source}</b><small>{l.assignedTo||"Unassigned"}{l.leadCost?` · $${l.leadCost.toFixed(2)}`:""}</small></span><span><em className={`stage ${l.stage.toLowerCase().replace(" ","-")}`}>{l.stage}</em></span><span>{l.outcome}<small>{l.attempts||0} attempt{l.attempts===1?"":"s"}</small></span><span>{nextAction?new Date(nextAction).toLocaleString():"—"}{l.doNotCall&&<strong className="dnc">DNC</strong>}</span></button>}),[filteredLeads,priorityNow]);
  const dueFollowUps=useMemo(()=>scheduledFollowUps(lineLeads,priorityNow),[lineLeads,priorityNow]);
  const dueLeadCount=dueFollowUps.length;
  function updateLead(id:number, patch:Partial<Lead>){if(patch.followUp!==undefined)patch={...patch,followUpUtc:patch.followUp&&Number.isFinite(Date.parse(patch.followUp))?new Date(patch.followUp).toISOString():""};setLeads(list=>list.map(l=>l.id===id?{...l,...patch}:l))}
  ...
  function openLeadMessage(leadItem:Lead,channel:"sms"|"email"){
    if(channel==="sms"&&!leadItem.phone){setToast("Add a phone number first");return}
    if(channel==="email"&&!leadItem.email){setToast("Add an email address first");return}
  ...
  return <main className="app-shell" onDragEnter={onFileDragEnter} onDragOver={onFileDragOver} onDragLeave={onFileDragLeave} onDrop={onFileDrop}>
    ...
    <aside className="sidebar">
      ...
      <nav>{nav.map(([id,label,icon])=><button key={id} className={view===id?"active":""} aria-label={id==="messages"&&messageUnreadCount?`${label}, ${messageUnreadCount} unread`:label} title={label} aria-current={view===id?"page":undefined} onClick={()=>openView(id)}><Icon name={icon}/><span>{label}</span>{id==="leads"&&<em>{leads.length}</em>}{id==="messages"&&messageUnreadCount>0&&<em className="message-unread">{Math.min(99,messageUnreadCount)}</em>}</button>)}</nav>
      <div className="sidebar-install"><PwaInstallButton/></div>
      <div className="sidebar-foot"><div className="agent"><span>{currentUserName.split(" ").map(part=>part[0]).slice(0,2).join("")||"P"}</span><div><b>{currentUserName}</b><small title={currentUserEmail}><i/> {userRole}</small></div></div>{isOwner&&<button aria-label="Owner settings" title="Settings" onClick={()=>setView("settings")}><Icon name="gear"/></button>}</div>
    </aside>
    ...
    <section className="workspace">
      ...
      {view==="dialer"&&<div className={`dialer-view conversation-workspace ${connected||postCallLeadId?"has-conversation":"is-waiting"} ${keypadOpen?"has-keypad":""}`}>
        ...
        {(connected||Boolean(postCallLeadId))&&<aside className="lead-file" aria-label="Current lead file">
          <header>
            ...
            <em className={connected?"live":""} role={postCallLeadId?"button":undefined} tabIndex={postCallLeadId?0:undefined} title={postCallLeadId?"Open call wrap-up":undefined} onClick={()=>{if(postCallLeadId)document.getElementById("post-call-wrap")?.scrollIntoView({block:"nearest"})}} onKeyDown={event=>{if(postCallLeadId&&(event.key==="Enter"||event.key===" ")){event.preventDefault();document.getElementById("post-call-wrap")?.scrollIntoView({block:"nearest"})}}}>{postCallLeadId?"WRAP UP":connected?"LIVE":callableLeads.length?`${leadQueuePosition+1} OF ${leadQueueTotal}`:"OPEN"}</em>
          </header>
          {lead.id&&!manualCall?<>
            ...
            {postCallLead?<section id="post-call-wrap" className="post-call-wrap" aria-label="Post-call wrap-up">
              ...
              <div className="post-call-fields">
                ...
                <label><span>Pacifica outcome</span><select value={postCallDraft.crmOutcome} onChange={event=>choosePostCallOutcome(event.target.value)}><option>No answer</option><option>Voicemail</option><option>Interested</option><option>Call back later</option><option>Appointment set</option><option>Not interested</option><option>Wrong number</option><option>Sold / Won</option><option>Completed</option></select></label>
      ...
      {view==="leads"&&<div className="page-view crm-view"><header className="module-bar"><span className="eyebrow">{queueLabel(activeLine,workspaceProfile.mode).toUpperCase()} CONTACTS</span><div className="module-actions"><button className="secondary" disabled={scanBusy} onClick={()=>scanInputRef.current?.click()}><Icon name="camera"/> {scanBusy?"Reading…":"Scan document"}</button><button className="secondary" onClick={()=>inputRef.current?.click()}><Icon name="upload"/> Import CSV</button><button className="secondary" disabled={!filteredLeads.length} title="Export contacts in this queue matching the current filters" onClick={()=>{downloadContactsCsv(contactsCsv(filteredLeads),`pacifica-${activeLine}-${new Date().toISOString().slice(0,10)}.csv`);setToast(`${filteredLeads.length} contacts exported`)}}><Icon name="download"/> Export CSV ({filteredLeads.length})</button><button className="primary" onClick={openNewLead}>+ Add lead</button></div></header>{importReport&&<div className="import-hint" role="status"><span><i/>Import summary</span><p>{importReport}</p></div>}<div className="crm-summary five"><article><span>CONTACTS</span><b>{lineLeads.length}</b></article><article><span>LEAD SPEND</span><b>${lineLeads.reduce((total,leadItem)=>total+leadItem.leadCost,0).toFixed(0)}</b></article><article><span>FOLLOW-UPS</span><b>{dueLeadCount}</b></article><article><span>APPOINTMENTS</span><b>{lineLeads.filter(l=>l.stage==="Appointment").length}</b></article><article><span>CALL ATTEMPTS</span><b>{lineLeads.reduce((total,item)=>total+(item.attempts||0),0)}</b></article></div><div className="crm-tools"><div className="crm-search-shell"><span aria-hidden="true">⌕</span><input className="crm-search-input" value={search} onChange={e=>setSearch(e.target.value)} aria-label="Search contacts" placeholder="Search name, phone or email"/></div><div className="contact-filter-fields"><label><span>Source</span><select aria-label="Filter by source" value={sourceFilter} onChange={e=>setSourceFilter(e.target.value)}>{sourceOptions.map(source=><option key={source}>{source}</option>)}</select></label><label><span>Owner</span><select aria-label="Filter by owner" value={ownerFilter} onChange={e=>setOwnerFilter(e.target.value)}>{ownerOptions.map(owner=><option key={owner}>{owner}</option>)}</select></label><label><span>Stage</span><select aria-label="Filter by contact stage" value={stageFilter} onChange={e=>setStageFilter(e.target.value)}><option>All stages</option><option>New lead</option><option>Follow-up</option><option>Appointment</option><option>Quoted</option><option>Closed</option></select></label><label><span>Sort</span><select aria-label="Sort contacts" value={leadSort} onChange={e=>setLeadSort(e.target.value)}><option value="Next best">Recommended order</option><option>Newest</option><option>Follow-up due</option><option>Recently attempted</option><option>Name</option></select></label></div></div><div className="table-card crm-table smart-table"><div className="table-head"><span>CONTACT & PRIORITY</span><span>SOURCE & OWNER</span><span>STAGE</span><span>LAST RESULT</span><span>NEXT FOLLOW-UP</span></div>{contactRows}{!filteredLeads.length&&<div className="empty-state">No matching contacts</div>}</div></div>}

      {view==="miner"&&workspaceProfile.mode==="insurance"&&<MinerPanel mode={minerMode} onMode={setMinerMode} prospects={minerLeads} dialing={dialing} activeScope={dialerScope} onImport={()=>minerInputRef.current?.click()} onContractorImport={file=>{setMinerMode("commercial");importFile(file,"commercial")}} onStart={startMinerDialer} onCall={callLeadById} onOpen={setSelectedLead} autoFeed={workspaceProfile.minerAutoFeed} onAutoFeedChange={minerAutoFeed=>setWorkspaceProfile(profile=>({...profile,minerAutoFeed}))}/>}
      ...
      <div hidden={view!=="ai"} className="page-view ai-view"><AiCommandCenter visible={view==="ai"} onActivity={setAiActivity} key={workspaceId} workspaceId={workspaceId} activeLine={activeLine} profile={workspaceProfile} leads={allLeads} recentCalls={callLogs} onApply={applyAiAction} onCreateLead={createAiLead} onOpen={id=>setSelectedLead(id)} onCall={callLeadById}/></div>
      ...
      {workspaceHydrated&&clerkEnabled&&<CalendarAlerts key={workspaceId} workspaceId={workspaceId} onOpen={()=>openView("office")}/>}
      {view==="office"&&<div className="page-view"><OfficeDesk workspaceId={workspaceId} leads={leads} profile={workspaceProfile} onProfile={setWorkspaceProfile} isOwner={isOwner} onOpen={setSelectedLead}/></div>}
      ...
      {view==="opportunities"&&<div className="page-view"><OpportunityDesk leads={leads} initialLeadId={quoteRequestLeadId} onOpen={setSelectedLead} onMessage={id=>{const item=leads.find(l=>l.id===id);if(item)openLeadMessage(item,"sms")}} onRefresh={async()=>{const response=await fetch("/api/crm/workspace",{cache:"no-store"});if(!response.ok)throw Error("Contact refresh failed. Your reviewed request remains saved.");const data=await response.json();setLeads(current=>mergeIncomingContacts(current,normalizeSavedLeads(data.leads)));}}/></div>}
      {view==="quotes"&&<div className="page-view"><IndustryTools mode={workspaceProfile.mode} onScan={openDocumentPicker} scanBusy={scanBusy} leads={leads} onCollect={id=>{setQuoteRequestLeadId(id);setView("opportunities")}} onOpen={setSelectedLead} onQuote={setGrowthLeadId} onQuoted={id=>{const lead=leads.find(l=>l.id===id);if(lead)updateLead(id,{stage:"Quoted",automationEnabled:false,automationNextAt:"",sourceDisposition:/smart\s*financial/i.test(lead.source)?"Quoted without Contact":"Quoted"});}}/></div>}
      ...
      {settingsSection==="billing"&&<div className="settings-section billing-view"><header className="module-bar"><span className="eyebrow">PLANS &amp; BILLING</span></header><div className="pricing-grid">
        <article><span>SOLO</span><h2>${pacificaPlans.solo.monthlyPrice}<small>/month</small></h2><p>{pacificaPlans.solo.description}</p><ul><li>{pacificaPlans.solo.seats}</li><li>{pacificaPlans.solo.numbers}</li><li>Complete lead management workspace</li><li>Power dialer and automatic call queue</li><li>Voice clarity controls</li><li>Performance reporting</li><li>AI assistance and quote preparation</li></ul><button disabled={Boolean(checkoutPlan)} onClick={()=>void subscribe("solo")}>{checkoutPlan==="solo"?"Opening secure checkout…":"Choose Solo"}</button></article>
        <article className="featured"><em>MOST POPULAR</em><span>TEAM</span><h2>${pacificaPlans.team.monthlyPrice}<small>/month</small></h2><p>{pacificaPlans.team.description}</p><ul><li>{pacificaPlans.team.seats}</li><li>{pacificaPlans.team.numbers}</li><li>Priority-based lead queues</li><li>Compliance safeguards and calling reports</li><li>Guided onboarding</li><li>Industry tools and lead intake</li></ul><button disabled={Boolean(checkoutPlan)} onClick={()=>void subscribe("team")}>{checkoutPlan==="team"?"Opening secure checkout…":"Choose Team"}</button></article>
        <article><span>AGENCY</span><h2>${pacificaPlans.agency.monthlyPrice}<small>/month</small></h2><p>{pacificaPlans.agency.description}</p><ul><li>{pacificaPlans.agency.seats}</li><li>{pacificaPlans.agency.numbers}</li><li>Multi-team campaigns and reporting</li><li>Caller reputation tools</li><li>Dedicated onboarding support</li><li>Industry tools and lead intake</li></ul><button disabled={Boolean(checkoutPlan)} onClick={()=>void subscribe("agency")}>{checkoutPlan==="agency"?"Opening secure checkout…":"Choose Agency"}</button></article>
    ...
    {postCallLead&&!floatingWindowOpen&&<PostCallDispositionModal lead={postCallLead} draft={postCallDraft} technicalOutcome={postCallTechnicalOutcome} connected={postCallConnected} resume={resumeAfterWrap} saving={sourceSyncing} onSelect={choosePostCallOutcome} onChange={patch=>setPostCallDraft(draft=>({...draft,...patch}))} onSave={()=>void savePostCall()} onCallAgain={()=>void savePostCall("again")} onPause={()=>{resumeAfterWrapRef.current=false;setResumeAfterWrap(false);stopAutoDial("Queue paused · save this call result when ready")}}/>}
    {deletedLead&&<div className="contact-deleted-notice" role="status"><span>{deletedLead.name} deleted</span><button type="button" onClick={undoDelete}>Undo</button><button type="button" aria-label="Dismiss deletion message" onClick={()=>setDeletedLead(null)}>×</button></div>}
    {activeLead&&<div className="drawer-backdrop" onClick={()=>setSelectedLead(null)}><aside ref={contactDialogRef} role="dialog" aria-modal="true" aria-label={`Contact details for ${activeLead.name}`} tabIndex={-1} className="contact-drawer" onClick={e=>e.stopPropagation()}><header><div className="drawer-person"><i>{activeLead.name.split(" ").map(x=>x[0]).slice(0,2).join("")}</i><span><small>{(activeLead.product||"Lead type not supplied").toUpperCase()} CONTACT</small><h2>{activeLead.name}</h2>{activeLead.lastCallResult&&<p role="status">Last call: {activeLead.lastCallResult}</p>}<p>{activeLead.phone} · {activeLead.city}</p></span></div><button aria-label="Close contact" onClick={()=>setSelectedLead(null)}>×</button></header><div className="record-actions"><button disabled={!activeLead.phone||activeLead.doNotCall} onClick={()=>loadLeadInDialer(activeLead)}><Icon name="dial"/> Call</button><button disabled={!activeLead.phone||activeLead.smsOptOut||activeLead.doNotCall} onClick={()=>openLeadMessage(activeLead,"sms")}><Icon name="chat"/> Text</button><button disabled={!activeLead.email||activeLead.emailOptOut||activeLead.doNotCall} onClick={()=>openLeadMessage(activeLead,"email")}><Icon name="mail"/> Email</button><button className="quote-action" onClick={()=>{setGrowthLeadId(activeLead.id);setSelectedLead(null)}}><Icon name="spark"/> Quote</button><button className={activeLead.priorityOverride==="high"?"pin-active":""} onClick={()=>updateLead(activeLead.id,{priorityOverride:activeLead.priorityOverride==="high"?"auto":"high"})}>{activeLead.priorityOverride==="high"?"Pinned":"Pin"}</button><button disabled={activeLead.stage==="Closed"} onClick={()=>updateLead(activeLead.id,{stage:"Closed",status:"Closed",outcome:"Not interested",followUp:"",automationEnabled:false,automationNextAt:"",automationStatus:"complete",automationUpdatedAt:new Date().toISOString()})}>{activeLead.stage==="Closed"?"Lead closed":"Close lead"}</button></div><section className="record-section"><span className="section-label">CONTACT DETAILS</span><div className="field-grid"><label>Name<input value={activeLead.name} onChange={e=>updateLead(activeLead.id,{name:e.target.value})}/></label><label>Phone<input value={activeLead.phone} onChange={e=>updateLead(activeLead.id,{phone:e.target.value})}/></label><label>Email<input value={activeLead.email} onChange={e=>updateLead(activeLead.id,{email:e.target.value})}/></label><label>City<input value={activeLead.city} onChange={e=>updateLead(activeLead.id,{city:e.target.value})}/></label><label>Lead queue<select value={activeLead.line} onChange={e=>moveLeadToQueue(activeLead,e.target.value as LeadLine)}><option value="life">{queueLabel("life",workspaceProfile.mode)}</option><option value="home-auto">{queueLabel("home-auto",workspaceProfile.mode)}</option></select></label><label className="sms-consent-field"><span>Text message consent</span><select value={activeLead.smsOptOut?"optout":activeLead.smsConsent?"yes":"no"} onChange={e=>updateLead(activeLead.id,e.target.value==="yes"?{smsConsent:true,smsOptOut:false}:e.target.value==="optout"?{smsConsent:false,smsOptOut:true}:{smsConsent:false,smsOptOut:false})}><option value="no">Not documented</option><option value="yes">Opted in</option><option value="optout">Opted out / STOP</option></select><small>Only select Opted in when you have documented permission to text.</small></label><label className="sms-consent-field"><span>Email consent</span><select aria-label="Email consent" value={activeLead.emailOptOut?"optout":activeLead.emailConsent?"yes":"no"} onChange={e=>updateLead(activeLead.id,e.target.value==="yes"?{emailConsent:true,emailOptOut:false}:e.target.value==="optout"?{emailConsent:false,emailOptOut:true}:{emailConsent:false,emailOptOut:false})}><option value="no">Not documented</option><option value="yes">Opted in</option><option value="optout">Unsubscribed</option></select><small>Record permission for email separately from text messages. Only restore an unsubscribe after a new documented opt-in.</small></label></div></section><section className="record-section"><span className="section-label">STAGE & OUTCOME</span><div className="field-grid"><label>Stage<select value={activeLead.stage} onChange={e=>updateLead(activeLead.id,{stage:e.target.value,status:e.target.value==="Closed"?"Closed":"Ready"})}><option>New lead</option><option>Follow-up</option><option>Appointment</option><option>Quoted</option><option>Closed</option></select></label><label>Call outcome<select value={activeLead.outcome} onChange={e=>{const outcome=e.target.value;const closed=outcome==="Not interested"||outcome==="Wrong number";const appointment=outcome==="Appointment set";updateLead(activeLead.id,{outcome,lastContact:"Just now",sourceDisposition:sourceDispositionForOutcome(activeLead.source,outcome,activeLead.sourceDisposition),stage:closed?"Closed":appointment?"Appointment":["Interested","Call back later"].includes(outcome)?"Follow-up":activeLead.stage,status:closed?"Closed":"Ready"})}}><option>Not contacted</option><option>No answer</option><option>Voicemail</option><option>Interested</option><option>Call back later</option><option>Appointment set</option><option>Not interested</option><option>Wrong number</option></select></label><label>Follow-up date<input type="datetime-local" value={activeLead.followUp} onChange={e=>updateLead(activeLead.id,{followUp:e.target.value,stage:e.target.value&&activeLead.stage==="New lead"?"Follow-up":activeLead.stage})}/></label><label>Last contact<input disabled value={activeLead.lastContact}/></label></div><div className="quick-follow-up"><span>QUICK FOLLOW-UP</span><button onClick={()=>updateLead(activeLead.id,{followUp:suggestedRetryAt(),stage:"Follow-up"})}>Later today</button><button onClick={()=>updateLead(activeLead.id,{followUp:followUpInDays(1),stage:"Follow-up"})}>Tomorrow</button><button onClick={()=>updateLead(activeLead.id,{followUp:followUpInDays(3),stage:"Follow-up"})}>In 3 days</button><button onClick={()=>updateLead(activeLead.id,{followUp:""})}>Clear</button></div></section>{Object.values(activeLead.importedFields||{}).some(value=>String(value).trim())&&<section className="record-section imported-record-section"><div className="imported-record-heading"><span className="section-label">IMPORTED CSV DATA</span><small>{Object.keys(activeLead.importedFields||{}).length} fields · {activeLead.csvFileName||"CSV import"}{activeLead.csvUpdatedAt?` · synced ${new Date(activeLead.csvUpdatedAt).toLocaleString()}`:""}</small></div><div className="imported-data-grid">{Object.entries(activeLead.importedFields||{}).map(([field,value])=><div key={field}><span>{field}</span><b>{value||"—"}</b></div>)}</div></section>}<section className="record-section"><span className="section-label">NOTES</span><textarea value={activeLead.notes} onChange={e=>updateLead(activeLead.id,{notes:e.target.value})} placeholder="Add conversation notes, needs, objections, or next steps…"/></section><section className="timeline"><span className="section-label">ACTIVITY</span><div><i/><span><b>{activeLead.outcome}</b><small>{activeLead.lastContact}</small></span></div>{activeLead.lastSmsAt&&<div><i className="navy"/><span><b>Text message sent</b><small>{new Date(activeLead.lastSmsAt).toLocaleString()}</small></span></div>}{activeLead.stage==="Closed"&&<div><i className="navy"/><span><b>Lead closed</b><small>Excluded from automatic dialing and follow-ups · manual calls available</small></span></div>}{activeLead.followUp&&<div><i className="amber"/><span><b>Follow-up scheduled</b><small>{new Date(activeLead.followUp).toLocaleString()}</small></span></div>}<div><i className="navy"/><span><b>Contact added to Pacifica CRM</b><small>{activeLead.importedAt?new Date(activeLead.importedAt).toLocaleDateString():"Date unavailable"}</small></span></div></section><footer><button type="button" className="delete-contact" onClick={()=>deleteLead(activeLead)}>Delete contact</button><button onClick={()=>void syncLeadDisposition(activeLead,{sourceDisposition:activeLead.sourceDisposition,stage:activeLead.stage,outcome:activeLead.outcome,followUp:activeLead.followUp,notes:activeLead.notes}).then(message=>{setToast(message);setSelectedLead(null)})}>Save contact</button></footer></aside></div>}
    {growthLead&&<div className="quote-modal-backdrop" onMouseDown={()=>setGrowthLeadId(null)}><div ref={quoteDialogRef} tabIndex={-1} className="quote-modal-shell" role="dialog" aria-modal="true" aria-label={`Quote workspace for ${growthLead.name}`} onMouseDown={event=>event.stopPropagation()}><button className="quote-modal-close" type="button" aria-label="Close quote workspace" onClick={()=>setGrowthLeadId(null)}>×</button><LeadGrowthPanel lead={growthLead} teamMembers={workspaceProfile.teamMembers} onPatch={patch=>{const won=patch.outcome==="Sold / Won";updateLead(growthLead.id,{...patch,status:won?"Closed":growthLead.status,sourceDisposition:won?sourceDispositionForOutcome(growthLead.source,"Sold / Won",growthLead.sourceDisposition):growthLead.sourceDisposition,closedAt:won?new Date().toISOString():growthLead.closedAt} as Partial<Lead>);if(won)setToast(`${growthLead.name} marked won`)}} onGoogleCalendar={()=>{const url=googleCalendarUrl(growthLead);if(url)window.open(url,"_blank","noopener,noreferrer");else setToast("Choose a follow-up date first")}} onDownloadCalendar={()=>downloadCalendar(growthLead)}/></div></div>}
