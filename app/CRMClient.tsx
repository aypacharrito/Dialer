...
function isDocumentImage(file:File){return ["image/jpeg","image/png","image/webp"].includes(file.type)||/\.(jpe?g|png|webp)$/i.test(file.name)}
function renderDocumentBitmap(bitmap:ImageBitmap,rotation:0|-90){
  const largest=Math.max(bitmap.width,bitmap.height);const scale=Math.min(1,1600/largest);const width=Math.max(1,Math.round(bitmap.width*scale));const height=Math.max(1,Math.round(bitmap.height*scale));const canvas=document.createElement("canvas");canvas.width=rotation?height:width;canvas.height=rotation?width:height;const context=canvas.getContext("2d");if(!context)throw new Error("The image could not be prepared");if(rotation){context.translate(0,canvas.height);context.rotate(-Math.PI/2)}context.drawImage(bitmap,0,0,width,height);return canvas.toDataURL("image/jpeg",.8);
}
async function documentImageDataUrls(file:File){
  ...
  const bitmap=await createImageBitmap(file);try{const images=[renderDocumentBitmap(bitmap,0)];if(bitmap.height>bitmap.width*1.08)images.push(renderDocumentBitmap(bitmap,-90));return images}finally{bitmap.close()}
}
...
export default function Page({clerkEnabled=false,isOwner=false,isPlatformOwner=false,workspaceId="local",currentUserName="Pacifica user",currentUserEmail="",userRole="owner"}:{clerkEnabled?:boolean;isOwner?:boolean;isPlatformOwner?:boolean;workspaceId?:string;currentUserName?:string;currentUserEmail?:string;userRole?:string}){
  ...
  async function scanDocument(file?:File){
    if(!file)return;if(!isDocumentImage(file)){setToast("Use a JPEG, PNG, or WebP photo");return}if(file.size>15*1024*1024){setToast("Use an image smaller than 15 MB");return}
    setScanBusy(true);setToast("Reading document…");
    try{const response=await fetch("/api/ai/document-lead",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({images:await documentImageDataUrls(file),fileName:file.name})});const data=await response.json() as {extraction?:DocumentLeadExtraction;error?:string};if(!response.ok||!data.extraction)throw new Error(data.error||"Document scan failed");const extraction=data.extraction;setNewLead({...emptyNewLead,name:documentLeadName(extraction),phone:extraction.phone,email:extraction.email,city:extraction.city,address:extraction.address,state:extraction.state,zip:extraction.zip,product:extraction.product||(/insurance|policy/i.test(extraction.documentType)?"Insurance quote":"Service inquiry"),source:"Document scan",dateOfBirth:extraction.dateOfBirth,licenseNumber:extraction.licenseNumber,licenseState:extraction.licenseState,licenseExpiration:extraction.licenseExpiration,policyNumber:extraction.policyNumber,policyEffectiveDate:extraction.policyEffectiveDate,policyExpirationDate:extraction.policyExpirationDate,vin:extraction.vin,vehicle:[extraction.vehicleYear,extraction.vehicleMake,extraction.vehicleModel].filter(Boolean).join(" "),documentType:extraction.documentType,documentFields:documentLeadImportedFields(extraction)});setShowNewLead(true);setToast("Document read · verify the fields and add a phone number")}
    catch(error){setToast(error instanceof Error?error.message:"Document scan failed")}finally{setScanBusy(false)}
  }
  function handleDroppedFile(file?:File){if(!file)return;if(isDocumentImage(file)){void scanDocument(file);return}if(/\.(csv|tsv|txt)$/i.test(file.name)||["text/csv","text/tab-separated-values","text/plain"].includes(file.type)){importFile(file);return}setToast("Drop a license photo, policy image, CSV, TSV, or TXT file")}
