...
import SalesHQ from "./components/SalesHQ";
...
export default function Page({clerkEnabled=false,isOwner=false,workspaceId="local"}:{clerkEnabled?:boolean;isOwner?:boolean;workspaceId?:string}){
  ...
  useEffect(()=>{let canceled=false;async function hydrate(){let localLeads:Lead[]=[];let localLogs:CallLog[]=[];const leadKey=`pacifica:${workspaceId}:leads`;const logKey=`pacifica:${workspaceId}:call-logs`;if(!clerkEnabled){try{localLeads=normalizeSavedLeads(JSON.parse(localStorage.getItem(leadKey)||"[]"));const parsedLogs=JSON.parse(localStorage.getItem(logKey)||"[]");localLogs=Array.isArray(parsedLogs)?parsedLogs:[]}catch{}}else{try{const response=await fetch("/api/crm/workspace",{cache:"no-store"});const data=await response.json() as {found?:boolean;leads?:unknown[];callLogs?:CallLog[]};if(response.ok&&data.found){localLeads=normalizeSavedLeads(data.leads);localLogs=Array.isArray(data.callLogs)?data.callLogs:[]}}catch{}}if(canceled)return;setLeads(localLeads);setCallLogs(localLogs);setWorkspaceHydrated(true)}void hydrate();return()=>{canceled=true}},[clerkEnabled,workspaceId]);
  useEffect(()=>{if(!workspaceHydrated)return;const leadKey=`pacifica:${workspaceId}:leads`;const logKey=`pacifica:${workspaceId}:call-logs`;try{localStorage.setItem(leadKey,JSON.stringify(leads));localStorage.setItem(logKey,JSON.stringify(callLogs.slice(0,500)))}catch{}if(!clerkEnabled)return;if(workspaceSaveTimerRef.current)window.clearTimeout(workspaceSaveTimerRef.current);workspaceSaveTimerRef.current=window.setTimeout(()=>{void fetch("/api/crm/workspace",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({leads,callLogs:callLogs.slice(0,500)})}).catch(()=>{})},600);return()=>{if(workspaceSaveTimerRef.current)window.clearTimeout(workspaceSaveTimerRef.current)}},[leads,callLogs,workspaceHydrated,clerkEnabled,workspaceId]);
  useEffect(()=>{leadsRef.current=leads},[leads]);
  // Provider secrets stay server-side. The authenticated owner checks the inbound queue.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(()=>{if(!workspaceHydrated||!isOwner)return;void syncLeadFeed(false);const timer=window.setInterval(()=>void syncLeadFeed(false),20000);return()=>window.clearInterval(timer)},[workspaceHydrated,isOwner]);
  useEffect(()=>{ fetch("/api/twilio/status").then(r=>r.json()).then(data=>{setPhoneReady(Boolean(data.configured));if(data.phoneNumber)setCallerId(String(data.phoneNumber));setPhoneStatus(data.configured?`${data.phoneNumber} ready over Wi-Fi`:"Secure API key still needed")}).catch(()=>setPhoneStatus("Unable to check Twilio setup")); return()=>{if(nextCallTimerRef.current)window.clearTimeout(nextCallTimerRef.current);deviceRef.current?.destroy();clearVoiceProcessorRef.current=null} },[]);
  ...
  function scheduleNextAuto(completedLeadId?:number){
    ...
    if(!queue.length||nextPosition>=queue.length){stopAutoDial(`${queueLabel(activeLineRef.current)} queue completed`);return}
  ...
  function openPostCall(leadId:number,resumeQueue:boolean){
    ...
    resumeAfterWrapRef.current=resumeQueue;autoDialRef.current=false;setAutoDialing(false);
    ...
    updateLead(leadId,{lastContact:new Date().toLocaleString()});
  }
  async function savePostCall(){
    ...
    let syncMessage="Saved in Pacifica";
    try{
      const response=await fetch("/api/integrations/dispositions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({source:completedLead.source,vendorId:completedLead.vendorId,leadName:completedLead.name,phone:completedLead.phone,disposition:postCallDraft.sourceDisposition,crmStage:postCallDraft.crmStage,crmOutcome:postCallDraft.crmOutcome,appointmentAt:postCallDraft.appointmentAt,notes:postCallDraft.notes})});
      ...
      syncMessage=data.message||(data.synced?`${completedLead.source} updated`:"Saved in Pacifica · source sync not connected");
      updateLead(completedLead.id,{sourceSyncStatus:syncMessage});
    }catch(error){syncMessage=`Saved in Pacifica · ${error instanceof Error?error.message:"source update failed"}`;updateLead(completedLead.id,{sourceSyncStatus:syncMessage})}
    const resume=resumeAfterWrapRef.current;resumeAfterWrapRef.current=false;setPostCallLeadId(null);setSourceSyncing(false);setToast(syncMessage);
  ...
  async function placeCall(number:string,wasManual:boolean,queuedLead:Lead=lead){
    ...
    currentLogRef.current={id:crypto.randomUUID(),name:wasManual?"Manual call":queuedLead.name,phone:number,startedAt:new Date().toISOString(),duration:0,outcome:"Dialing",status:"Connecting",campaign:queueLabel(queuedLead.line),source:wasManual?"Manual keypad":"CRM auto dial"};
    try{
  ...
  function start(){ if(postCallLeadId){setToast("Save the connected call result before continuing");return} if(!callableLeads.length){setView("leads");setToast(lineLeads.length?"No open contacts are eligible to dial":`Import ${queueLabel(activeLine)} first`);return} autoDialRef.current=true;setAutoDialing(true);void placeCall(lead.phone,false,lead) }
  ...
  function pauseQueue(){resumeAfterWrapRef.current=false;stopAutoDial("Auto dialing paused — finish the current call or wrap-up safely")}
  ...
  function dispositionFields(disposition:string){
    const value=disposition.toLowerCase();
    if(value.includes("appointment"))return {stage:"Appointment",outcome:"Appointment set"};
    if(value.includes("sold")||value.includes("closed"))return {stage:"Closed",outcome:"Completed"};
    if(value.includes("not interested")||value.includes("lost"))return {stage:"Closed",outcome:"Not interested"};
  ...
  function importFile(file?: File) {
    ...
    reader.onload = () => {
      ...
      const rawHeader = parseRow(first, delimiter);
      const header = rawHeader.map((value) => value.toLowerCase().replace(/[^a-z0-9]/g, ""));
      ...
      const nameIndex = find("prospect", "business", "businessname", "company", "companyname", "name", "fullname", "customername", "leadname", "contactname");
      ...
      const queueIndex = find("queue", "leadqueue", "callqueue", "segment");
      const priorityIndex = find("priority", "leadpriority", "tier");
      const followUpIndex = find("followup", "followupdate", "nextfollowup", "nextcontact");
      const dncIndex = find("donotcall", "dnc");
      const smsConsentIndex = find("smsconsent", "textconsent");
      const knownIndexes = new Set([firstNameIndex,lastNameIndex,nameIndex,phoneIndex,emailIndex,typeIndex,dispositionIndex,costIndex,cityIndex,notesIndex,sourceIndex,addressIndex,stateIndex,zipIndex,territoryIndex,brandIndex,profileIndex,receivedIndex,returnIndex,employeesIndex,searchProIndex,vendorIdIndex,queueIndex,priorityIndex,followUpIndex,dncIndex,smsConsentIndex].filter(index=>index>=0));
      ...
      rows.slice(hasHeader ? 1 : 0).forEach((row, rowIndex) => {
        ...
        const queueValue = hasHeader ? `${get(cells, queueIndex)} ${get(cells, priorityIndex)}`.toLowerCase() : "";
        const line: LeadLine = /priority|high|tier\s*a|\ba\b/.test(queueValue)?"life":/general|medium|tier\s*b|\bb\b/.test(queueValue)?"home-auto":activeLine;
        ...
        if(hasHeader)rawHeader.forEach((label,columnIndex)=>{const field=label.trim();const value=get(cells,columnIndex);if(field&&value&&!knownIndexes.has(columnIndex))extraFields[field]=value});
        if(get(cells,priorityIndex))extraFields.Priority=get(cells,priorityIndex);
        if(get(cells,queueIndex))extraFields.Queue=get(cells,queueIndex);
        ...
        parsed.push({
          ...
          followUp: get(cells, followUpIndex),
          doNotCall: /^(1|true|yes|y)$/i.test(get(cells,dncIndex)),
          lastContact: "Never",
          line,
          ...
          extraFields,
          smsConsent: /^(1|true|yes|y|opted in)$/i.test(get(cells,smsConsentIndex)),
        });
      });
  ...
  const nav:[View,string,string][]=isOwner?[["sales","Sales HQ","spark"],["dialer","Dialer","dial"],["leads","CRM contacts","users"],["messages","Messages","chat"],["ai","Pacifica AI","spark"],["quotes","Industry tools","shield"],["campaigns","Pipeline","list"],["activity","Reports","chart"],["billing","Plans & billing","list"]]:[["dialer","Dialer","dial"],["leads","CRM contacts","users"],["messages","Messages","chat"],["ai","Pacifica AI","spark"],["quotes","Industry tools","shield"],["campaigns","Pipeline","list"],["activity","Reports","chart"],["billing","Plans & billing","list"]];
  ...
  const filteredLeads=lineLeads.filter(l=>(stageFilter==="All stages"||l.stage===stageFilter)&&(sourceFilter==="All sources"||l.source===sourceFilter)&&`${l.name} ${l.phone} ${l.email} ${l.city} ${l.source} ${l.product}`.toLowerCase().includes(search.toLowerCase()));
  function updateLead(id:number, patch:Partial<Lead>){setLeads(list=>list.map(l=>l.id===id?{...l,...patch}:l))}
  function addContact(){
    const id=Date.now();
    const contact:Lead={...emptyLead,id,name:"New contact",phone:"",city:"",status:"Ready",line:activeLine,source:"Manual entry",importedAt:new Date().toISOString()};
    setLeads(list=>[contact,...list]);setSelectedLead(id);setToast("New contact created");
  }
  ...
  async function resetCurrentWorkspace(){
    if(!window.confirm("Reset only this signed-in account? This permanently deletes its leads and call history without affecting other Pacifica accounts."))return;
    if(!window.confirm("This cannot be undone. Delete this account's workspace now?"))return;
    try{
      const response=await fetch("/api/crm/workspace",{method:"DELETE"});
      const data=await response.json().catch(()=>({})) as {error?:string};
      if(!response.ok)throw new Error(data.error||"Workspace reset failed");
      setLeads([]);setCallLogs([]);setSelectedLead(null);setIndex(0);
      localStorage.removeItem(`pacifica:${workspaceId}:leads`);localStorage.removeItem(`pacifica:${workspaceId}:call-logs`);
      setToast("This account's workspace was reset");
    }catch(error){setToast(error instanceof Error?error.message:"Workspace reset failed")}
  }

  return <main className="app-shell">
    <aside className="sidebar">
      <div className="logo"><span className="brand-mark"><Image src="/pacifica-mark.png" width={39} height={39} alt=""/></span><div><b>PACIFICA</b></div></div>
        ...
        <section className={`hero-call ${connected?"connected":""}`}>
          ...
          <div className="call-grid">
            ...
            <div className="call-controls">
              ...
              {!dialing?<button className="start-call" onClick={start}><Icon name="play"/><span>{autoDialing?"Call next lead":`Start ${queueLabel(activeLine)} queue`}</span></button>:<button className="end-call" onClick={hangup}><Icon name="end"/><span>{connected?"Hang up":"End attempt"}</span></button>}
        ...
        <aside className="lead-file" aria-label="Current lead file">
          ...
          {lead.id&&!manualCall?<>
            ...
            {postCallLeadId===lead.id?<section className="post-call-wrap" aria-label="Post-call wrap-up">
              ...
              <footer><small>{lead.sourceSyncStatus||`Pacifica will save this result. ${lead.source||"Lead-source"} sync runs after its status API is connected.`}</small><button disabled={sourceSyncing} onClick={()=>void savePostCall()}>{sourceSyncing?"Saving…":resumeAfterWrapRef.current?"Save & call next":"Save result"}</button></footer>
