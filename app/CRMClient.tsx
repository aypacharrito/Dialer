...
export default function Page({clerkEnabled=false,isOwner=false,workspaceId="local"}:{clerkEnabled?:boolean;isOwner?:boolean;workspaceId?:string}){
  ...
  function importFile(file?: File) {
    ...
    reader.onload = () => {
      ...
      rows.slice(hasHeader ? 1 : 0).forEach((row, rowIndex) => {
        ...
        const line: LeadLine = activeLine;
  ...
  function createLead(){const phone=newLead.phone.trim();const name=newLead.name.trim();if(!name||phone.replace(/\D/g,"").length<7){setToast("Add a name and complete phone number");return}const item:Lead={id:Date.now(),name,phone,email:newLead.email.trim(),city:newLead.city.trim()||"No city",status:"Ready",stage:"New lead",outcome:"Not contacted",notes:"",followUp:"",doNotCall:false,lastContact:"Never",line:activeLine,source:newLead.source.trim()||"Manual",leadCost:Number(newLead.leadCost)||0,product:newLead.product.trim()||"Service inquiry",sourceDisposition:"New",importedAt:new Date().toISOString(),smsConsent:false,smsOptOut:false};setLeads(old=>[item,...old]);setNewLead({name:"",phone:"",email:"",city:"",product:"",source:"Manual",leadCost:""});setShowNewLead(false);setSelectedLead(item.id);setView("leads");setToast(`${name} added to ${queueLabel(activeLine)}`)}
