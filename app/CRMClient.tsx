...
export default function Page({clerkEnabled=false,isOwner=false,isPlatformOwner=false,workspaceId="local",currentUserName="Pacifica user",currentUserEmail="",userRole="owner"}:{clerkEnabled?:boolean;isOwner?:boolean;isPlatformOwner?:boolean;workspaceId?:string;currentUserName?:string;currentUserEmail?:string;userRole?:string}){
  ...
  useEffect(()=>{if(!workspaceHydrated)return;const leadKey=`pacifica:${workspaceId}:leads`;const logKey=`pacifica:${workspaceId}:call-logs`;const profileKey=`pacifica:${workspaceId}:profile`;try{localStorage.setItem(leadKey,JSON.stringify(allLeads));localStorage.setItem(logKey,JSON.stringify(callLogs.slice(0,500)));localStorage.setItem(profileKey,JSON.stringify(workspaceProfile))}catch{}if(!clerkEnabled){queueMicrotask(()=>setWorkspaceSyncStatus("Saved in this browser"));return}if(workspaceSaveTimerRef.current)window.clearTimeout(workspaceSaveTimerRef.current);queueMicrotask(()=>setWorkspaceSyncStatus("Saving changes…"));workspaceSaveTimerRef.current=window.setTimeout(()=>{workspaceSaveTimerRef.current=undefined;void workspaceSaveQueue.queue.save(JSON.stringify({leads:allLeads,callLogs:callLogs.slice(0,500),profile:workspaceProfile})).then(latest=>{if(latest)setWorkspaceSyncStatus(`Saved ${new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"})}`)}).catch(()=>setWorkspaceSyncStatus("Offline · automatic retry pending"))},600);return()=>{if(workspaceSaveTimerRef.current){window.clearTimeout(workspaceSaveTimerRef.current);workspaceSaveTimerRef.current=undefined}}},[allLeads,callLogs,workspaceProfile,workspaceHydrated,clerkEnabled,workspaceId,workspaceSavePulse,workspaceSaveQueue]);
  ...
  const matchingLeads=lineLeads.filter(l=>(stageFilter==="All stages"||l.stage===stageFilter)&&(sourceFilter==="All sources"||l.source===sourceFilter)&&(ownerFilter==="All owners"||(ownerFilter==="Unassigned"?!l.assignedTo:l.assignedTo===ownerFilter))&&contactMatches(l,search));
  ...
  const dueFollowUps=scheduledFollowUps(lineLeads,priorityNow);
