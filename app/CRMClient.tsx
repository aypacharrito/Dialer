...
export default function Page({clerkEnabled=false,isOwner=false,isPlatformOwner=false,workspaceId="local",currentUserName="Pacifica user",currentUserEmail="",userRole="owner"}:{clerkEnabled?:boolean;isOwner?:boolean;isPlatformOwner?:boolean;workspaceId?:string;currentUserName?:string;currentUserEmail?:string;userRole?:string}){
  ...
  const [importReport,setImportReport]=useState("Upload or re-upload any lead CSV. Pacifica matches duplicates, enriches existing contacts, and keeps every source column searchable.");
  ...
  useEffect(()=>{if(!workspaceHydrated||!clerkEnabled||view!=="activity")return;let canceled=false;const refresh=()=>{void fetch("/api/crm/workspace",{cache:"no-store"}).then(response=>response.json()).then(data=>{if(!canceled&&Array.isArray(data.callLogs))setCallLogs(current=>mergeRecordingUpdates(current,data.callLogs as CallLog[]))}).catch(()=>undefined)};const initial=window.setTimeout(refresh,0);const timer=window.setInterval(refresh,5000);return()=>{canceled=true;window.clearTimeout(initial);window.clearInterval(timer)}},[clerkEnabled,view,workspaceHydrated]);
    ...
    <section className="workspace">
      ...
      {view==="dialer"&&<div className="dialer-view"><div className="dialer-main-grid">
        <section className={`hero-call ${connected?"connected":""}`}>
          <div className="hero-head"><div><span className="eyebrow">PACIFICA · {queueLabel(activeLine,workspaceProfile.mode).toUpperCase()} DIALER</span><h1>{postCallLeadId?"Complete the call summary.":connected?"Call connected.":dialing?"Calling through Twilio…":autoDialing?"Preparing the next contact…":"Ready to start calling."}</h1><p>{postCallLeadId?"Record the result, notes, and next action before Pacifica continues the queue.":connected?"Speak normally. When the call ends, save the result and Pacifica will prepare the next contact.":dialing?"Pacifica is placing the call and will connect you when the contact answers.":`Start with the highest-priority contact in your ${queueLabel(activeLine,workspaceProfile.mode)} queue.`}</p></div><div className="dialer-behavior-note"><b>{autoDialing?"AUTO DIAL ACTIVE":"QUEUE READY"}</b><span>Use the keypad for one-off calls.</span></div></div>
          <div className="call-grid">
            ...
            <div className="call-controls">
              ...
              {!dialing?<button className="start-call" onClick={start}><Icon name="play"/><span>{autoDialing?"Call next contact":"Start calling"}</span></button>:<button className="end-call" onClick={hangup}><Icon name="end"/><span>{connected?"End call":"Cancel call"}</span></button>}
        ...
        <section className="bottom-grid">
          ...
          <article className="queue-card"><header><div><span>{queueLabel(activeLine,workspaceProfile.mode).toUpperCase()} · UP NEXT</span><b>{callableLeads.length?`${Math.max(0,callableLeads.length-(dialing?1:0))} open contacts remaining`:"Queue is empty"}</b></div><button onClick={()=>setView("leads")}>View CRM</button></header>{upNextLeads.map((l,i)=><div className="queue-row" key={l.id}><em>{String(i+1).padStart(2,"0")}</em><span className="mini-avatar">{l.name.split(" ").map(x=>x[0]).slice(0,2).join("")}</span><div><b>{l.name}</b><small>{l.phone} · {l.city}</small></div><span className="ready">READY</span></div>)}{!callableLeads.length&&<div className="empty-queue"><b>{lineLeads.length?"No contacts need automatic dialing":"No contacts yet"}</b><span>{lineLeads.length?"Interested, working, appointed, closed, and do-not-call contacts stay safely in the CRM without re-entering this queue.":`Import a CSV or TXT file into the ${queueLabel(activeLine,workspaceProfile.mode)} CRM.`}</span>{!lineLeads.length&&<button onClick={()=>inputRef.current?.click()}>Import contacts</button>}</div>}</article>
        </section>
      </div>}

      {view==="leads"&&<div className="page-view crm-view"><div className="page-title"><div><span className="eyebrow">{queueLabel(activeLine,workspaceProfile.mode).toUpperCase()} CONTACTS</span><h1>Work every lead from one clear view.</h1><p>Import, organize, prioritize, and follow up without losing any information from your original lead file.</p></div><button className="primary" onClick={()=>inputRef.current?.click()}><Icon name="upload"/> Import contacts</button></div><div className="import-hint"><span>SMART CSV IMPORT</span><p>{importReport}</p></div><div className="crm-summary five"><article><span>TOTAL CONTACTS</span><b>{lineLeads.length}</b></article><article><span>LEAD INVESTMENT</span><b>${lineLeads.reduce((total,leadItem)=>total+leadItem.leadCost,0).toFixed(0)}</b></article><article><span>FOLLOW-UPS DUE</span><b>{dueLeadCount}</b></article><article><span>APPOINTMENTS SET</span><b>{lineLeads.filter(l=>l.stage==="Appointment").length}</b></article><article><span>REVENUE WON</span><b>${lineLeads.reduce((total,item)=>total+(item.closedRevenue||0),0).toFixed(0)}</b></article></div><div className="crm-tools"><label><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search names, phone numbers, sources, notes, or imported fields"/></label><select aria-label="Filter by source" value={sourceFilter} onChange={e=>setSourceFilter(e.target.value)}>{sourceOptions.map(source=><option key={source}>{source}</option>)}</select><select aria-label="Filter by owner" value={ownerFilter} onChange={e=>setOwnerFilter(e.target.value)}>{ownerOptions.map(owner=><option key={owner}>{owner}</option>)}</select><select aria-label="Filter by pipeline stage" value={stageFilter} onChange={e=>setStageFilter(e.target.value)}><option>All stages</option><option>New lead</option><option>Follow-up</option><option>Appointment</option><option>Closed</option></select><select aria-label="Sort contacts" value={leadSort} onChange={e=>setLeadSort(e.target.value)}><option value="Next best">Recommended order</option><option>Newest</option><option>Follow-up due</option><option>Recently attempted</option><option>Name</option></select></div><div className="table-card crm-table smart-table"><div className="table-head"><span>CONTACT & PRIORITY</span><span>SOURCE & OWNER</span><span>PIPELINE STAGE</span><span>LAST RESULT</span><span>NEXT FOLLOW-UP</span></div>{filteredLeads.map(l=>{const priority=leadPriority(l,priorityNow);const nextAction=l.followUp||l.automationNextAt;return <button className="table-row" key={l.id} onClick={()=>setSelectedLead(l.id)}><span><i>{l.name.split(" ").map(x=>x[0]).slice(0,2).join("")}</i><span><b>{l.name}</b><small>{l.phone||"No phone"} · {l.email||"No email"}</small><small className="priority-summary"><em className={`priority-pill ${priority.level.toLowerCase()}`}>{priority.level}</em>{priority.reason}</small></span></span><span className="lead-source"><b>{l.source}</b><small>{l.assignedTo||"Unassigned"}{l.leadCost?` · $${l.leadCost.toFixed(2)}`:""}</small></span><span><em className={`stage ${l.stage.toLowerCase().replace(" ","-")}`}>{l.stage}</em></span><span>{l.outcome}<small>{l.attempts||0} attempt{l.attempts===1?"":"s"}</small></span><span>{nextAction?new Date(nextAction).toLocaleString():"—"}{l.doNotCall&&<strong className="dnc">DNC</strong>}</span></button>})}{!filteredLeads.length&&<div className="empty-state">No {queueLabel(activeLine,workspaceProfile.mode)} contacts match those filters.</div>}</div></div>}
      ...
      {view==="billing"&&<div className="page-view billing-view"><div className="page-title"><div><span className="eyebrow">PLANS & BILLING</span><h1>Choose the plan built for your team.</h1><p>Secure monthly billing through Stripe, with dedicated business numbers assigned after account verification.</p></div></div><div className="pricing-grid">
      ...
      {isOwner&&view==="settings"&&<div className="page-view settings-page">
        <div className="settings-title-bar">
          <div><span className="eyebrow">OWNER SETTINGS</span><h1>One organized place to run your workspace.</h1><p>Manage business details, teammates, calling, integrations, and launch readiness without digging through one long configuration screen.</p></div>
        ...
        <div className="settings-layout">
          <nav className="settings-nav" aria-label="Settings sections">
            ...
            {([
              ["workspace","01","Workspace","Brand, queues, appearance"],
              ["team","02","Team & routing","Access and assignments"],
              ["phone","03","Calling","Audio, numbers, voice setup"],
              ["integrations","04","Integrations","Lead delivery and email"],
              ["system","05","System health","Launch checks and status"],
            ] as const).map(([section,number,label,detail])=><button key={section} type="button" className={settingsSection===section?"active":""} aria-current={settingsSection===section?"page":undefined} onClick={()=>setSettingsSection(section)}><i>{number}</i><span><b>{label}</b><small>{detail}</small></span></button>)}
          </nav>
          <div className="settings-content">
            {settingsSection==="workspace"&&<section className="settings-section"><div className="settings-section-intro"><div><span>WORKSPACE</span><h2>Identity and preferences</h2><p>Choose how this account looks, how its queues are named, and which business details appear in customer communication.</p></div><em>AUTOSAVED</em></div><WorkspaceProfileSettings profile={workspaceProfile} onChange={setWorkspaceProfile}/></section>}
            {settingsSection==="team"&&<section className="settings-section"><div className="settings-section-intro"><div><span>TEAM & ROUTING</span><h2>People and ownership</h2><p>Give each teammate their own login and decide how new opportunities are assigned.</p></div><em>PRIVATE WORKSPACE</em></div><TeamManagement profile={workspaceProfile} onChange={setWorkspaceProfile}/></section>}
            {settingsSection==="phone"&&<section className="settings-section"><div className="settings-section-intro"><div><span>CALLING</span><h2>Phone and audio</h2><p>Test the salesperson’s microphone and speakers, then manage the private number assigned to this workspace.</p></div><em className={phoneReady?"ready":"attention"}>{phoneReady?"VOICE READY":"SETUP NEEDED"}</em></div><div className="phone-setup-layout"><PhoneSettings ensureDevice={ensureDevice}/><div className="phone-setup-side"><div className="provider-card"><div><span className="eyebrow">VOICE PROVIDER</span><h2>{callerId}</h2><p>Secure browser calling, inbound availability, microphone checks, and runtime error reporting.</p></div><div className={`twilio-selected ${phoneReady?"":"waiting"}`}><i/> {phoneReady?"CONFIGURED":"NEEDS ATTENTION"}</div></div><article className="setup-help"><span>TWILIO VOICE WEBHOOK</span><code>https://pacificacrm.com/api/twilio/voice</code><p>HTTP POST · Redeploy Production after changing a Vercel environment variable.</p><a href="/api/twilio/diagnostics" target="_blank" rel="noreferrer">Open safe diagnostics ↗</a></article><PhoneWorkspaceSetup phoneNumber={callerId} workspaceId={workspaceId}/></div></div>{isPlatformOwner&&<PhoneNumberCenter currentWorkspaceId={workspaceId} onAssignmentChange={()=>void refreshPhoneStatus()}/>}</section>}
            {settingsSection==="integrations"&&<section className="settings-section settings-integrations"><div className="settings-section-intro"><div><span>INTEGRATIONS</span><h2>Lead delivery and email</h2><p>Connect outside lead sources and prepare customer email without exposing provider secrets to the browser.</p></div><em>SERVER SECURED</em></div><article className="smart-connect"><header><div><span>UNIVERSAL LEAD DELIVERY</span><h2>Send any provider’s leads into Pacifica</h2></div><strong className="connected"><i/> READY</strong></header><p>Give the lead company these connection details. Replace PROVIDER_NAME so every contact keeps its original source.</p><dl className="lead-api-details"><div><dt>Posting URL</dt><dd>https://pacificacrm.com/api/integrations/leads?workspace={workspaceId}&amp;source=PROVIDER_NAME</dd></div><div><dt>Method</dt><dd>POST · application/json</dd></div><div><dt>Header name</dt><dd>x-pacifica-webhook-secret</dd></div><div><dt>Header value</dt><dd>LEAD_WEBHOOK_SECRET from Vercel</dd></div></dl><p>The secret remains on the server. Pacifica checks this account’s private inbound queue every 20 seconds.</p><div><button onClick={saveLeadFeedConnection}>Check for leads</button><small>{leadFeedStatus}</small></div></article><EmailWorkspaceSetup/></section>}
            {settingsSection==="system"&&<section className="settings-section"><div className="settings-section-intro"><div><span>SYSTEM HEALTH</span><h2>Production readiness</h2><p>See what is live, what still needs provider credentials, and whether automations completed safely.</p></div><em>LIVE CHECKS</em></div><SystemHealthPanel/></section>}
          </div>
        </div>
      </div>}
    </section>
