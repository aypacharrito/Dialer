...
export default function Page({clerkEnabled=false,isOwner=false,isPlatformOwner=false,workspaceId="local",currentUserName="Pacifica user",currentUserEmail="",userRole="owner"}:{clerkEnabled?:boolean;isOwner?:boolean;isPlatformOwner?:boolean;workspaceId?:string;currentUserName?:string;currentUserEmail?:string;userRole?:string}){
  ...
  return <main className="app-shell" onDragEnter={onFileDragEnter} onDragOver={onFileDragOver} onDragLeave={onFileDragLeave} onDrop={onFileDrop}>
    ...
    <aside className="sidebar">
      ...
      <nav>{nav.map(([id,label,icon])=>{const pending=pendingViews.has(id);return <button key={id} className={pending?"nav-pending":view===id?"active":""} disabled={pending} aria-label={pending?`${label} coming soon`:id==="messages"&&messageUnreadCount?`${label}, ${messageUnreadCount} unread`:label} title={pending?`${label} · coming soon`:label} aria-current={view===id?"page":undefined} onClick={()=>openView(id)}><Icon name={icon}/><span>{label}</span>{id==="leads"&&<em>{leads.length}</em>}{id==="messages"&&messageUnreadCount>0&&<em className="message-unread">{Math.min(99,messageUnreadCount)}</em>}{pending&&<small className="nav-soon">SOON</small>}</button>})}</nav>
