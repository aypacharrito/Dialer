...
import { mergeCsvLeads, normalizedCsvEmail, normalizedCsvPhone } from "./lib/csv-lead-merge";
...
export default function Page({clerkEnabled=false,isOwner=false,isPlatformOwner=false,workspaceId="local",currentUserName="Pacifica user",currentUserEmail="",userRole="owner"}:{clerkEnabled?:boolean;isOwner?:boolean;isPlatformOwner?:boolean;workspaceId?:string;currentUserName?:string;currentUserEmail?:string;userRole?:string}){
  ...
  const callableLeads=useMemo(()=>rankLeads(lineLeads.filter(item=>item.stage!=="Closed"&&!item.doNotCall&&normalizedCsvPhone(item.phone).length>=7),priorityNow),[lineLeads,priorityNow]);
  ...
  function importFile(file?: File) {
    ...
    reader.onload = () => {
      ...
      if (parsed.length) {
        ...
        if(merged.added||merged.updated)setLeads(merged.leads);
        setIndex(0);
        const summary = `${merged.added.toLocaleString()} new · ${merged.updated.toLocaleString()} enriched · ${merged.unchanged.toLocaleString()} already current${merged.matched?` · ${merged.matched.toLocaleString()} duplicates safely merged`:""}${invalid?` · ${invalid} invalid rows`:""}`;
