    ...
    for(const model of modelCandidates()){
      try{
        const response=await client.responses.create({
          model,
          store:false,
          input:[{role:"system",content:"You are Pacifica AI, a careful CRM copilot for lead-driven sales and service teams. Analyze only the provided records. Never invent facts, prices, consent, eligibility, promises, or appointments. Do not recommend contacting do-not-call records. Keep advice concise and relevant to the product or service shown in each record. Proposed changes require human approval."},{role:"user",content:`Request: ${prompt}\n\nCRM records:\n${JSON.stringify(leads)}`}],
