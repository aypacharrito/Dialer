...
type CrmAnalysis={summary:string;priorities:CrmPriority[];actions:CrmAction[];draft:string;subject?:string;channel?:"sms"|"email";recipientIds?:number[];createLead:CrmCreateLead|null;mode?:"ai"|"smart-fallback";notice?:string};
...
export async function POST(request:Request){
  ...
  try{
    const body=await request.json() as {prompt?:string;includeNotes?:boolean;leads?:Array<Record<string,unknown>>;recentCalls?:Array<Record<string,unknown>>;images?:unknown;history?:Array<{role:string;content:string}>};
    ...
    try{
      const response=await client.responses.create({
        ...
        },required:["summary","priorities","actions","draft","subject","channel","recipientIds","createLead"]}}},
