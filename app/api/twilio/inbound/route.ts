...
export async function POST(request:Request){
  ...
  try{
    ...
    const leads=(workspace.leads as Array<Record<string,unknown>>).map(raw=>{
      ...
      return {...raw,lastInboundAt:sentAt,automationEnabled:false,automationUpdatedAt:sentAt,lastContact:optedOut?"STOP received · outreach closed":"Text reply received",automationNextAt:"",automationStatus:optedOut?"opted out":"replied",...(optedOut?{smsOptOut:true,smsConsent:false,doNotCall:true,stage:"Closed",status:"Closed",outcome:"Not interested",sourceDisposition:"Lost - Not Interested",followUp:""}:optedIn?{smsOptOut:false,smsConsent:true}:{}),communications:appendCommunication(raw.communications,{id:sid,channel:"sms",direction:"inbound",body,status:"received",sentAt,provider:"twilio",providerId:sid})};
    });
    if(!matched){const sentAt=new Date().toISOString(),optedOut=stop.test(body);leads.unshift({id:Date.now(),name:`Inbound text · ${from.slice(-4)}`,phone:from,email:"",city:"",status:optedOut?"Closed":"Ready",stage:optedOut?"Closed":"New lead",outcome:optedOut?"Not interested":"Not contacted",notes:"Created automatically from an inbound text.",followUp:"",doNotCall:optedOut,lastContact:optedOut?"STOP received · outreach closed":"Text reply received",line:"life",source:"Inbound SMS",leadCost:0,product:"Inbound inquiry",sourceDisposition:optedOut?"Lost - Not Interested":"New",importedAt:sentAt,received:sentAt,smsConsent:!optedOut,smsOptOut:optedOut,lastInboundAt:sentAt,automationEnabled:false,automationUpdatedAt:sentAt,automationNextAt:"",automationStatus:optedOut?"opted out":"replied",communications:[{id:sid,channel:"sms",direction:"inbound",body,status:"received",sentAt,provider:"twilio",providerId:sid}]})}
