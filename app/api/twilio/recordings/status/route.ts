...
export async function POST(request:Request){
  ...
  try{
    const workspace=await readStoredWorkspace(workspaceId);if(!workspace)return Response.json({received:true,ignored:true});const recordingUrl=`/api/twilio/recordings?sid=${sid}`;
    let matched=false;const callLogs=workspace.callLogs.map(raw=>{const log=raw as Record<string,unknown>;if(log.callSid!==callSid)return log;matched=true;return {...log,recordingSid:sid,recordingUrl,recordingStatus:status}});
    if(!matched){const lead=workspace.leads.find(raw=>Number((raw as Record<string,unknown>).id)===leadId) as Record<string,unknown>|undefined;callLogs.unshift({id:`recording-${callSid}`,callSid,name:String(lead?.name||"Recorded call"),phone:String(lead?.phone||""),startedAt:new Date().toISOString(),duration:0,outcome:"Recording received",status:"Recording received",campaign:"Pacifica",source:"Twilio callback",recordingSid:sid,recordingUrl,recordingStatus:status})}
