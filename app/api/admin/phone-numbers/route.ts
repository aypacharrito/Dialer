...
export async function POST(request:Request){
  ...
  try{
    ...
    if(body.action==="sms-status"){
      ...
      return Response.json({ok:true,assignment:saved,message:`SMS marked registered for ${saved.phoneNumber}. ${process.env.TWILIO_A2P_APPROVED==="true"?"SMS sending is enabled.":"Sending is still paused. Set TWILIO_A2P_APPROVED=true in Vercel for Production, then redeploy."}`});
    }
