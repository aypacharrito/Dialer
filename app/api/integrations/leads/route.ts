...
function flattenPayload(payload:unknown):IncomingRecord {
  ...
  const nested=[root.lead,root.data,root.prospect,root.contact].find(value=>value&&typeof value==="object"&&!Array.isArray(value));
...
async function ensureInboundTable() {
  ...
  const db=getD1();
  await db.prepare(`CREATE TABLE IF NOT EXISTS inbound_leads (id TEXT PRIMARY KEY, vendor_id TEXT, source TEXT NOT NULL DEFAULT 'Lead provider', name TEXT NOT NULL, phone TEXT NOT NULL, phone_digits TEXT NOT NULL UNIQUE, email TEXT NOT NULL DEFAULT '', city TEXT NOT NULL DEFAULT 'Imported', product TEXT NOT NULL DEFAULT 'Home & Auto', line TEXT NOT NULL DEFAULT 'home-auto', disposition TEXT NOT NULL DEFAULT 'Received - not worked yet', notes TEXT NOT NULL DEFAULT '', cost REAL NOT NULL DEFAULT 0, created_at TEXT NOT NULL, synced_at INTEGER NOT NULL DEFAULT 0)`).run();
  return db;
}
