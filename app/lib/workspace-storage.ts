...
function workspaceIdentityKeys(lead:Record<string,unknown>){const phone=String(lead.phone||"").replace(/\D/g,"").slice(-10),email=String(lead.email||"").trim().toLowerCase(),vendor=String(lead.vendorId||"").trim(),source=String(lead.source||"").trim().toLowerCase();return [phone.length>=7?`phone:${phone}`:"",email.includes("@")?`email:${email}`:"",vendor?`vendor:${source}:${vendor}`:""].filter(Boolean)}
function workspaceDeletionTime(lead:Record<string,unknown>){const time=Date.parse(String(lead.deletionUpdatedAt||lead.deletedAt||""));return Number.isFinite(time)?time:0}
function workspaceDeletionDecisions(leads:Array<Record<string,unknown>>){const map=new Map<string,Record<string,unknown>>();for(const lead of leads){if(!workspaceDeletionTime(lead))continue;for(const key of workspaceIdentityKeys(lead)){const previous=map.get(key);if(!previous||workspaceDeletionTime(lead)>=workspaceDeletionTime(previous))map.set(key,lead)}}return map}
function protectWorkspaceLead(lead:Record<string,unknown>,decisions:Map<string,Record<string,unknown>>){let latest:Record<string,unknown>|undefined;for(const key of workspaceIdentityKeys(lead)){const candidate=decisions.get(key);if(candidate&&(!latest||workspaceDeletionTime(candidate)>=workspaceDeletionTime(latest)))latest=candidate}return latest?{...lead,...deletionState(lead as {deletedAt?:string;deletionUpdatedAt?:string},latest as {deletedAt?:string;deletionUpdatedAt?:string})}:lead}
