...
import type {QuoteLink} from "./quote-intake";
export type QuoteCredentials={workspaceId:string;linkId:string;link?:QuoteLink};
function key(){
...
export function createQuoteToken(workspaceId:string,linkId:string,link?:QuoteLink){
 const iv=randomBytes(12),cipher=createCipheriv('aes-256-gcm',key(),iv);
 const data=Buffer.concat([cipher.update(JSON.stringify({workspaceId,linkId,...(link?{link}:{})}),'utf8'),cipher.final()]);
 return Buffer.concat([iv,cipher.getAuthTag(),data]).toString('base64url');
}
export function readQuoteToken(token:string):QuoteCredentials{
