...
function fieldValue(lead:TemplateLead,name:string){
  const normalized=name.trim().toLowerCase();
  const buckets=[lead.importedFields||{},lead.extraFields||{},lead];
  for(const bucket of buckets){for(const [key,value] of Object.entries(bucket)){if(key.trim().toLowerCase()!==normalized)continue;if(["string","number","boolean"].includes(typeof value))return String(value)}}
  return "";
}
...
export function renderCommunicationTemplate(value:string,lead:TemplateLead,profile:WorkspaceProfile){
  ...
  const replacements:Record<string,string>={
    ...
    "{{industry}}":industryLabels[profile.industry]||"sales",
    "{{value_proposition}}":profile.valueProposition||"",
    "{{sales_objective}}":profile.salesObjective||"",
  ...
  let result=Object.entries(replacements).reduce((output,[token,replacement])=>replaceAll(output,token,replacement),value);
  result=result.replace(/\{\{field:([^}]+)\}\}/gi,(_match,field)=>fieldValue(lead,String(field)));
  return result.replace(/ +([.,!?])/g,"$1").replace(/[ \t]+\n/g,"\n").trim();
}

export function templatizeCommunication(value:string,lead:TemplateLead,profile:WorkspaceProfile){
  let result=value;
  const replacements:Array<[string,string]>=[
    [profile.emailSignature,"{{email_signature}}"],[profile.callbackNumber,"{{callback_number}}"],[profile.businessName,"{{business_name}}"],[profile.agentName,"{{agent_name}}"],[profile.valueProposition,"{{value_proposition}}"],[lead.product,"{{product}}"],[lead.city,"{{city}}"],[lead.name,"{{full_name}}"],[lead.name.trim().split(/\s+/)[0]||"","{{first_name}}"],
  ];
