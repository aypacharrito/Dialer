export type OfficeItem={id:string;leadId:number;kind:'appointment'|'payment';title:string;dueAt:string;amount:number;status:'open'|'done';reminderAt:string;reminderState:'off'|'pending'|'sending'|'sent'|'review';reminderError?:string;providerId?:string;createdAt:string;durationMinutes?:number;staffReminderMinutes?:number};
...
export function officeStaffReminderAt(item:OfficeItem){
 if(item.status!=="open"||item.staffReminderMinutes===-1)return 0;
 return Date.parse(item.dueAt)-(item.staffReminderMinutes??15)*60000;
}
