...
test('minimize survives call timer updates; call results appear outside the CRM',()=>{
 ...
 update(event,{active:false,wrapUp:{id:'1:10'}});assert.equal(overlay.minimized,false);assert.equal(overlay.visible,true);assert.equal(overlay.bounds.height,500);
