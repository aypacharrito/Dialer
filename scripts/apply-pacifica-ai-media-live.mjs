...
// PACIFICA_LIVE_MIC_TYPESCRIPT_FIX_V1
// HTMLMediaElement supports setting attributes, but TypeScript's HTMLAudioElement
// interface does not expose a writable `playsInline` property. Keep the browser
// behavior without tripping Next.js type checking.
const phoneFile="app/components/PhoneSettings.tsx";
const phoneTarget=path.join(root,phoneFile);
if(fs.existsSync(phoneTarget)){
  let phoneSource=fs.readFileSync(phoneTarget,"utf8");
  phoneSource=phoneSource.replace(/audio\.playsInline\s*=\s*true;/g,'audio.setAttribute("playsinline","");');
  fs.writeFileSync(phoneTarget,phoneSource);
}

console.log("Pacifica AI image-to-contact CRM binding + live mic TypeScript fix applied.");
