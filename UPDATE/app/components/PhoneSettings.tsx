...
export default function PhoneSettings({ ensureDevice, compact = false, onClose }: {
  ...
  return <section ref={dialogRef} role={compact?"dialog":undefined} aria-modal={compact||undefined} aria-label={compact?"Communication devices":undefined} tabIndex={compact?-1:undefined} className={`phone-config ${compact?"compact":""}`}>
    ...
    <section className={`clearvoice-card ${clearVoiceEnabled?"enabled":""}`}>
      <div className="clearvoice-head"><div><span>AUDIO</span><h3>ClearVoice</h3></div><label className="clearvoice-switch"><input type="checkbox" checked={clearVoiceEnabled} onChange={event=>void updateClearVoice(event.target.checked)}/><i/><b>{clearVoiceEnabled?"ON":"OFF"}</b></label></div>
      ...
      <div className={`clearvoice-live-monitor ${listening?"active":""}`}><span>{listening?listening==="clearvoice"?`ClearVoice · ${clearVoiceModeLabels[clearVoiceMode]}`:"Microphone live":"Live monitor"}</span><button type="button" className={listening?"stop":""} aria-pressed={Boolean(listening)} onClick={()=>void toggleLiveMonitor()}>{listening?"Stop live monitor":"Start live monitor"}</button></div>
